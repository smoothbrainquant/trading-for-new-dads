#!/usr/bin/env python3
"""
Backtest for INVERTED Carry Factor Strategy with Funding Subtraction

This script backtests an INVERTED carry factor strategy that:
1. FLIPS the positions from the original carry strategy:
   - Long position: Coins with HIGHEST (most positive) funding rates -> we PAY funding
   - Short position: Coins with LOWEST (most negative) funding rates -> we PAY funding
2. SUBTRACTS funding costs from raw price returns
3. Tests if price momentum/trend can overcome funding rate headwinds

This is the opposite of the traditional carry trade which tries to collect funding.
Instead, this strategy pays funding and tries to profit from price movements.
"""
...
def rank_by_funding_rate_inverted(funding_df, date, top_n=10, bottom_n=10):
    """
    Rank coins by funding rate on a specific date - INVERTED STRATEGY.
    
    Instead of the traditional carry:
    - Long: HIGHEST funding rates (we pay funding, bet on price momentum)
    - Short: LOWEST funding rates (we pay funding, bet against price momentum)

    Args:
        funding_df (pd.DataFrame): Funding rates data
        date (pd.Timestamp): Date to rank on
        top_n (int): Number of top (highest funding rate) coins to LONG (inverted)
        bottom_n (int): Number of bottom (lowest funding rate) coins to SHORT (inverted)

    Returns:
        dict: Dictionary with 'long' (highest funding) and 'short' (lowest funding) lists
    """
    ...
    # INVERTED: Long coins with HIGHEST funding rates (pay funding, bet on momentum)
    long_symbols = date_data.tail(top_n)["coin_symbol"].tolist()

    # INVERTED: Short coins with LOWEST funding rates (pay funding, bet against them)
    short_symbols = date_data.head(bottom_n)["coin_symbol"].tolist()

    return {"long": long_symbols, "short": short_symbols}


def calculate_portfolio_returns_with_funding_subtraction(weights, returns_df, funding_df, current_date):
    """
    Calculate portfolio returns by SUBTRACTING funding costs from price returns.
    
    For each position:
    - If long with positive funding rate: pay funding (subtract from returns)
    - If long with negative funding rate: receive funding (add to returns, but we subtract so it becomes a cost)
    - If short with positive funding rate: receive funding (but we're inverted, so we pay)
    - If short with negative funding rate: pay funding (inverted strategy)
    
    Simplified: We ALWAYS subtract the funding rate impact as a cost.
    Net return = Price return - |Weight| * Funding_rate
    ...
    Args:
        ...
        funding_df (pd.DataFrame): DataFrame with funding rates
        current_date (pd.Timestamp): Current date for funding rate lookup

    Returns:
        tuple: (portfolio_return, funding_cost)
    """
    ...
    for symbol, weight in weights.items():
        ...
        if len(symbol_return) > 0 and not np.isnan(symbol_return[0]):
            price_return = weight * symbol_return[0]
        else:
            price_return = 0.0
            
        # Get funding rate (as daily rate: 8-hour rate * 3)
        fr = funding_df[
            (funding_df["date"] == current_date) & (funding_df["coin_symbol"] == symbol)
        ]["funding_rate_pct"].values
        
        funding_cost = 0.0
        if len(fr) > 0 and not np.isnan(fr[0]):
            # Funding rate is 8-hour, multiply by 3 for daily
            # We SUBTRACT funding cost from returns
            # Positive weight (long) + positive FR = we pay (negative to returns)
            # Negative weight (short) + positive FR = we receive (but inverted, we pay)
            funding_cost = abs(weight) * (fr[0] / 100.0) * 3.0  # Always a cost
            
        portfolio_return += price_return - funding_cost
        total_funding_cost += funding_cost

    return portfolio_return, total_funding_cost
...
def backtest(
    ...
    """
    Run backtest for the INVERTED carry factor strategy with funding subtraction.

    Args:
        price_data (pd.DataFrame): Historical OHLCV data
        funding_data (pd.DataFrame): Historical funding rates data
        top_n (int): Number of highest funding rate coins to LONG (inverted)
        bottom_n (int): Number of lowest funding rate coins to SHORT (inverted)
    ...
    if len(common_dates) < volatility_window + 10:
        raise ValueError(
            f"Insufficient overlapping data. Need at least {volatility_window + 10} days, have {len(common_dates)}"
        )
    ...
    print(f"  ??  INVERTED STRATEGY:")
    print(f"    - Long positions: {top_n} coins (HIGHEST funding rates - we pay funding)")
    print(f"    - Short positions: {bottom_n} coins (LOWEST funding rates - we pay funding)")
    print(f"    - Returns: Price returns MINUS funding costs")
    ...
    for date_idx, current_date in enumerate(daily_tracking_dates):
        ...
        if is_rebalance_day:
            # Rank coins by funding rate on this date (INVERTED)
            selected = rank_by_funding_rate_inverted(
        ...
        if current_weights and date_idx < len(daily_tracking_dates) - 1:
            ...
            # Calculate returns WITH funding subtraction
            portfolio_return, daily_funding_cost = calculate_portfolio_returns_with_funding_subtraction(
                current_weights, next_returns, funding_data, current_date
            )
    ...
    if not trades_df.empty:
        long_trades = trades_df[
            (trades_df["position_type"] == "long") & (trades_df["new_weight"] > 0)
        ]
        short_trades = trades_df[
            (trades_df["position_type"] == "short") & (trades_df["new_weight"] < 0)
        ]

        metrics["avg_long_funding_rate"] = (
            long_trades["funding_rate_pct"].mean() if len(long_trades) > 0 else 0
        )
        metrics["avg_short_funding_rate"] = (
            short_trades["funding_rate_pct"].mean() if len(short_trades) > 0 else 0
        )
        metrics["total_funding_cost"] = portfolio_df["daily_funding_cost"].sum()

    return {
        "portfolio_values": portfolio_df,
        "trades": trades_df,
        "metrics": metrics,
        "strategy_info": {"top_n": top_n, "bottom_n": bottom_n, "rebalance_days": rebalance_days},
    }
...
def print_results(results):
    ...
    print("\n" + "=" * 80)
    print("INVERTED CARRY FACTOR BACKTEST RESULTS")
    print("(Flipped Positions + Funding Subtracted from Returns)")
    print("=" * 80)

    print(f"\nStrategy Information:")
    print(f"  ??  INVERTED CARRY STRATEGY:")
    print(f"    - Long positions: {strategy_info['top_n']} coins with HIGHEST funding rates")
    print(f"    - Short positions: {strategy_info['bottom_n']} coins with LOWEST funding rates")
    print(f"    - We PAY funding on both sides")
    print(f"    - Funding costs SUBTRACTED from price returns")
    ...
    if "avg_long_funding_rate" in metrics:
        print(f"\nFunding Rate Statistics (COSTS, not income):")
        print(f"  Avg Long FR (we pay):   {metrics['avg_long_funding_rate']:>15.4f}%")
        print(f"  Avg Short FR (we pay):  {metrics['avg_short_funding_rate']:>15.4f}%")
        print(f"  Total Funding Cost:     ${metrics['total_funding_cost']:>15,.2f}")
...
def save_results(results, output_prefix="backtest_carry_factor_inverted"):
    """
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Backtest INVERTED Carry Factor Trading Strategy (Flipped + Funding Subtraction)",
    ...
    parser.add_argument(
        "--top-n", type=int, default=10, help="Number of highest funding rate coins to LONG (inverted)"
    )
    parser.add_argument(
        "--bottom-n", type=int, default=10, help="Number of lowest funding rate coins to SHORT (inverted)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--output-prefix",
        type=str,
        default="backtest_carry_factor_inverted",
    ...
    print("=" * 80)
    print("INVERTED CARRY FACTOR BACKTEST")
    print("(Flipped Positions + Funding Costs Subtracted)")
    print("=" * 80)
    print(f"\nConfiguration:")
    print(f"  Price data file: {args.price_data}")
    print(f"  Funding data file: {args.funding_data}")
    print(f"  ??  INVERTED STRATEGY:")
    print(f"    - Long positions: {args.top_n} coins (HIGHEST FR - we pay funding)")
    print(f"    - Short positions: {args.bottom_n} coins (LOWEST FR - we pay funding)")
