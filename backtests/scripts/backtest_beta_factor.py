...
def calculate_rolling_beta(data, btc_data, window=90):
    ...
    def calculate_beta(group):
        ...
        rolling_cov = (
            ...
            .cov(group["btc_return"])
        )
        rolling_var = (
            group["btc_return"].rolling(window=window, min_periods=int(window * 0.7)).var()
        )
...
def rank_by_beta(data, date, num_quintiles=5):
    ...
    try:
        ...
        date_data["quintile"] = pd.cut(
            date_data["beta_rank"], bins=num_quintiles, labels=range(1, num_quintiles + 1)
        )

    return date_data


def select_symbols_by_beta(
    data,
    date,
...
def run_backtest(
    ...
    for date_idx, current_date in enumerate(trading_dates):
        ...
        portfolio_beta = 0
        for symbol, weight in current_positions.items():
            symbol_beta_data = beta_data[
                (beta_data["date"] == current_date) & (beta_data["symbol"] == symbol)
            ]
    ...
    strategy_info = {
        ...
        "avg_long_beta": (
            trades_df[trades_df["signal"] == "LONG"]["beta"].mean()
            if not trades_df.empty
            else np.nan
        ),
        "avg_short_beta": (
            trades_df[trades_df["signal"] == "SHORT"]["beta"].mean()
            if not trades_df.empty
            else np.nan
        ),
        "long_symbols": (
...
def main():
    ...
    parser.add_argument(
        "--beta-window", type=int, default=90, help="Beta calculation window in days"
    )
    parser.add_argument(
    ...
    parser.add_argument("--num-quintiles", type=int, default=5, help="Number of beta quintiles")
    parser.add_argument(
