...
def calculate_rolling_trendline(data, window=30):
    ...
    trendline_df = (
        df.groupby("symbol", group_keys=False).apply(calculate_for_group).reset_index(drop=True)
    )
...
def calculate_trendline_score(df, method="multiplicative", r2_threshold=0.0):
    ...
    if method == "multiplicative":
        ...
        df["trendline_score"] = np.where(df["r_squared"] >= r2_threshold, df["norm_slope"], 0.0)
...
def filter_universe(
    data, min_volume=5_000_000, min_market_cap=50_000_000, min_r2=0.0, max_pvalue=1.0
):
    """
...
def rank_by_trendline(data, date, num_quintiles=5):
    ...
    try:
        ...
        date_data["quintile"] = pd.cut(
            date_data["trendline_rank"], bins=num_quintiles, labels=range(1, num_quintiles + 1)
        )

    return date_data


def select_symbols_by_trendline(
    data,
    date,
...
def run_backtest(
    ...
    print(
        f"  Slope range: [{trendline_data['slope'].min():.4f}, {trendline_data['slope'].max():.4f}]"
    )
    print(
        f"  R² range: [{trendline_data['r_squared'].min():.4f}, {trendline_data['r_squared'].max():.4f}]"
    )
    ...
    print(
        f"  Normalized slope range: [{trendline_data['norm_slope'].min():.2f}%, {trendline_data['norm_slope'].max():.2f}%]"
    )
    ...
    trendline_data = calculate_trendline_score(
        trendline_data, method=score_method, r2_threshold=r2_threshold
    )
    print(
        f"  Trendline score range: [{trendline_data['trendline_score'].min():.2f}, {trendline_data['trendline_score'].max():.2f}]"
    )
    ...
    if len(trading_dates) < trendline_window + rebalance_days:
        raise ValueError(
            f"Insufficient data. Need at least {trendline_window + rebalance_days} days."
        )
    ...
    for date_idx, current_date in enumerate(trading_dates):
        ...
        for symbol, weight in current_positions.items():
            symbol_data = trendline_data[
                (trendline_data["date"] == current_date) & (trendline_data["symbol"] == symbol)
            ]
            if not symbol_data.empty:
    ...
    strategy_info = {
        ...
        "avg_trendline_score_long": (
            trades_df[trades_df["signal"] == "LONG"]["trendline_score"].mean()
            if not trades_df.empty
            else np.nan
        ),
        "avg_trendline_score_short": (
            trades_df[trades_df["signal"] == "SHORT"]["trendline_score"].mean()
            if not trades_df.empty
            else np.nan
        ),
        "avg_r2_long": (
            trades_df[trades_df["signal"] == "LONG"]["r_squared"].mean()
            if not trades_df.empty
            else np.nan
        ),
        "avg_r2_short": (
            trades_df[trades_df["signal"] == "SHORT"]["r_squared"].mean()
            if not trades_df.empty
            else np.nan
        ),
        "long_symbols": (
...
def main():
    ...
    parser.add_argument(
        "--strategy",
        type=str,
        default="long_uptrend_short_downtrend",
        choices=[
            "long_uptrend_short_downtrend",
            "long_only_uptrends",
            "slope_only_momentum",
            "r2_only_quality",
        ],
    ...
    parser.add_argument(
        "--r2-threshold", type=float, default=0.0, help="Minimum R² for filtering (0.0 = no filter)"
    )
    parser.add_argument(
        "--pvalue-threshold",
        type=float,
    ...
    parser.add_argument(
        "--num-quintiles", type=int, default=5, help="Number of trendline quintiles"
    )
    parser.add_argument(
        "--long-percentile", type=int, default=80, help="Percentile threshold for long positions"
    )
    parser.add_argument(
        "--short-percentile", type=int, default=20, help="Percentile threshold for short positions"
    )
    parser.add_argument(
        "--weighting-method",
        type=str,
