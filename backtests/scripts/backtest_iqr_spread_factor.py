...
def calculate_iqr_spread(data, date, lookback=30, universe_size=100, 
...
def select_baskets(data, date, majors_count=10, small_caps_range=(51, 100),
                   min_volume=1_000_000, min_market_cap=10_000_000):
    """
    ...
    # Select majors (top N)
    majors = date_data.iloc[:majors_count].copy() if len(date_data) >= majors_count else pd.DataFrame()
    
    # Select small caps (rank start to end)
    start_rank = small_caps_range[0] - 1  # Convert to 0-indexed
    end_rank = small_caps_range[1]
    small_caps = date_data.iloc[start_rank:end_rank].copy() if len(date_data) >= end_rank else pd.DataFrame()
...
def run_backtest(data, strategy='dynamic_rotation', lookback_days=30, 
                rebalance_days=7, iqr_history_window=180, majors_count=10,
                small_caps_range=(51, 100), low_spread_threshold=20,
                high_spread_threshold=80, allocation_method='fixed',
                basket_weighting='equal_weight', initial_capital=10000,
                min_volume=1_000_000, min_market_cap=10_000_000,
    ...
    for date in trading_dates:
        iqr_stats = calculate_iqr_spread(
            data, date, lookback=lookback_days,
            universe_size=100, min_volume=min_volume,
    ...
    print(f"  IQR spread range: [{iqr_df['iqr_spread'].min():.2f}, {iqr_df['iqr_spread'].max():.2f}]")
    ...
    for date_idx, current_date in enumerate(trading_dates):
        ...
        regime = classify_regime(
            current_iqr, iqr_history,
            low_spread_threshold, high_spread_threshold
        )
        ...
        if current_date in rebalance_dates:
            ...
            baskets = select_baskets(
                data, current_date, majors_count=majors_count,
            ...
            for symbol, weight in current_positions.items():
                ...
                if not symbol_data.empty:
                    ...
                    if not np.isnan(daily_return):
                        ...
                        if not baskets['majors'].empty and symbol in baskets['majors']['symbol'].values:
                            majors_pnl += position_pnl
                        elif not baskets['small_caps'].empty and symbol in baskets['small_caps']['symbol'].values:
        ...
        # Calculate exposures
        majors_exposure = sum([w for sym, w in current_positions.items() 
                              if not baskets['majors'].empty and sym in baskets['majors']['symbol'].values])
        small_caps_exposure = sum([w for sym, w in current_positions.items() 
    ...
    iqr_df = iqr_df.merge(regime_dates_df[['date', 'regime', 'iqr_percentile']], 
                          on='date', how='left')
    ...
    regime_performance = calculate_regime_performance(
        portfolio_df, basket_performance_df
    )
...
def main():
    ...
    # Strategy parameters
    parser.add_argument('--strategy', type=str, default='dynamic_rotation',
                       choices=['dynamic_rotation', 'long_short', 'conditional_smallcap', 
                               'spread_mean_reversion'],
                       help='Strategy variant')
    
    # IQR calculation parameters
    parser.add_argument('--lookback-days', type=int, default=30,
                       help='Days for return calculation')
    parser.add_argument('--iqr-history-window', type=int, default=180,
                       help='Days of IQR history for regime classification')
    
    # Universe parameters
    parser.add_argument('--majors-count', type=int, default=10,
                       help='Number of coins in majors basket')
    parser.add_argument('--small-caps-start', type=int, default=51,
                       help='Starting rank for small caps')
    parser.add_argument('--small-caps-end', type=int, default=100,
                       help='Ending rank for small caps')
    
    # Regime thresholds
    parser.add_argument('--low-spread-threshold', type=int, default=20,
                       help='Percentile threshold for low spread regime')
    parser.add_argument('--high-spread-threshold', type=int, default=80,
    ...
    parser.add_argument('--allocation-method', type=str, default='fixed',
                       choices=['fixed', 'continuous', 'threshold'],
                       help='Allocation method')
    parser.add_argument('--basket-weighting', type=str, default='equal_weight',
    ...
    parser.add_argument('--min-market-cap', type=float, default=10_000_000,
