...
def calculate_sharpe_weights(backtest_results_file, output_file=None):
    ...
    for idx, row in results.iterrows():
        print(
            f"  {row['Strategy']:<20} | Sharpe: {row['Sharpe Ratio']:>8.3f} | Return: {row['Avg Return']:>8.2%}"
        )
    ...
    for idx, row in weights_df.iterrows():
        print(
            f"  {row['Strategy']:<20} | Sharpe: {row['Sharpe Ratio']:>8.3f} | Weight: {row['Weight']:>8.4f} ({row['Weight_Pct']:>6.2f}%)"
        )
