...
def calculate_z_scores_for_period(data, period, zscore_lookback=30):
    ...
    df[f"return_zscore_{period}d"] = (df[return_col] - df[f"return_{period}d_mean"]) / df[
        f"return_{period}d_std"
    ]
    df[f"volume_zscore_{period}d"] = (df[volume_col] - df[f"volume_{period}d_mean"]) / df[
        f"volume_{period}d_std"
    ]
...
def run_period_comparison(
    data, periods, zscore_lookback=30, return_threshold=-1.5, volume_threshold=1.0
):
    """
...
def print_detailed_analysis(results_df):
    ...
    pivot = fwd_1d.pivot_table(
        index="period", columns="signal_type", values="mean_return", aggfunc="first"
    )
...
def save_results(results_df, detailed_data, output_prefix="backtest_mean_reversion_periods"):
    ...
    period_cols = [
        col
        for col in detailed_data.columns
        if "return_" in col or "volume_" in col or "signal_" in col or "forward_" in col
    ]
...
def main():
    ...
    parser.add_argument(
        "--zscore-lookback", type=int, default=30, help="Lookback window for calculating z-scores"
    )
    parser.add_argument(
        "--return-threshold",
        type=float,
