#!/usr/bin/env python3
"""
Backtest for Durbin-Watson Factor Strategy

This script backtests a Durbin-Watson factor strategy that:
1. Calculates rolling Durbin-Watson statistic for all cryptocurrencies
2. Ranks cryptocurrencies by DW values (autocorrelation measure)
3. Creates long/short portfolios based on DW rankings:
   - Contrarian (Primary): Long high DW (mean reverting), Short low DW (momentum)
   - Momentum Continuation: Long low DW (trending), Short high DW (choppy)
   - Sweet Spot: Long moderate DW, Short extreme DW
...
DW hypothesis: The Durbin-Watson statistic measures autocorrelation in returns.
DW ≈ 2 = random walk, DW < 2 = momentum, DW > 2 = mean reversion.
Tests whether autocorrelation patterns predict future returns.
"""
...
def calculate_durbin_watson(returns, window=30):
    """
    Calculate Durbin-Watson statistic for a series of returns.
    ...
    if len(clean_returns) < window * 0.7:  # Require 70% data availability
        return np.nan
    ...
    # DW should be between 0 and 4, clip to avoid data quality issues
    if dw < 0 or dw > 4.5:
        return np.nan
...
def calculate_rolling_dw(data, window=30, method='raw_returns'):
    """
    ...
    Args:
        ...
        window (int): Rolling window size for DW calculation
        method (str): 'raw_returns' or 'residuals' (market-adjusted)
        
    Returns:
        pd.DataFrame: DataFrame with DW and supporting columns
    """
    df = data.copy()
    df = df.sort_values(['symbol', 'date']).reset_index(drop=True)
    ...
    print(f"  Calculating DW using method: {method}")
    
    if method == 'raw_returns':
        # Calculate DW on raw returns
        df['dw'] = df.groupby('symbol')['daily_return'].transform(
            lambda x: x.rolling(window=window, min_periods=int(window*0.7)).apply(
                calculate_durbin_watson, raw=False, kwargs={'window': window}
            )
        )
    
    elif method == 'residuals':
        # Calculate DW on residuals from BTC market model
        # Get BTC returns
        btc_data = df[df['symbol'].isin(['BTC', 'BTC/USD'])][['date', 'daily_return']].copy()
        btc_data.columns = ['date', 'btc_return']
        btc_data = btc_data.dropna()
        
        # Merge BTC returns
        df = df.merge(btc_data, on='date', how='left')
        
        # Calculate residuals from market model: r_coin = alpha + beta * r_btc + epsilon
        def calculate_dw_residuals(group):
            """Calculate DW on residuals for a group (one coin)"""
            def dw_from_residuals(returns_window, btc_window):
                mask = ~(np.isnan(returns_window) | np.isnan(btc_window))
                if mask.sum() < window * 0.7:
                    return np.nan
                
                coin_clean = returns_window[mask]
                btc_clean = btc_window[mask]
                
                if len(coin_clean) < 2:
                    return np.nan
                
                # Run simple regression
                if np.var(btc_clean) == 0:
                    residuals = coin_clean - np.mean(coin_clean)
                else:
                    beta = np.cov(coin_clean, btc_clean)[0,1] / np.var(btc_clean)
                    alpha = np.mean(coin_clean) - beta * np.mean(btc_clean)
                    residuals = coin_clean - (alpha + beta * btc_clean)
                
                # Calculate DW on residuals
                return calculate_durbin_watson(pd.Series(residuals), window=len(residuals))
            
            # Apply rolling calculation
            dw_series = pd.Series(index=group.index, dtype=float)
            for i in range(window, len(group) + 1):
                idx = group.index[i-1]
                returns_window = group.iloc[i-window:i]['daily_return'].values
                btc_window = group.iloc[i-window:i]['btc_return'].values
                dw_series.loc[idx] = dw_from_residuals(returns_window, btc_window)
            
            return dw_series
        
        df['dw'] = df.groupby('symbol', group_keys=False).apply(
            calculate_dw_residuals
        ).values
    
    else:
        raise ValueError(f"Unknown method: {method}. Use 'raw_returns' or 'residuals'")
    ...
    # Calculate autocorrelation for analysis
    df['autocorr_30d'] = df.groupby('symbol')['daily_return'].transform(
        lambda x: x.rolling(window=window, min_periods=int(window*0.7)).apply(
            lambda y: y.autocorr() if len(y.dropna()) >= 2 else np.nan
        )
    )
...
def rank_by_dw(data, date, num_quintiles=5):
    """
    Rank cryptocurrencies by Durbin-Watson statistic on a specific date.
    
    Args:
        data (pd.DataFrame): DataFrame with dw column
    ...
    # Remove NaN DW values
    date_data = date_data.dropna(subset=['dw'])
    
    # Filter extreme DW values (data quality issues)
    date_data = date_data[(date_data['dw'] >= 0.5) & (date_data['dw'] <= 3.5)]
    
    if len(date_data) < num_quintiles:
        return pd.DataFrame()
    
    # Rank by DW (ascending: low to high)
    date_data['dw_rank'] = date_data['dw'].rank(method='first', ascending=True)
    date_data['percentile'] = date_data['dw_rank'] / len(date_data) * 100
    
    # Assign quintiles (1 = lowest DW/momentum, num_quintiles = highest DW/mean reversion)
    try:
...
def select_symbols_by_dw(data, date, strategy='contrarian', 
                         num_quintiles=5, long_percentile=80, short_percentile=20):
    """
    Select symbols based on Durbin-Watson factor strategy.
    
    Args:
        data (pd.DataFrame): Data with dw column
        date (pd.Timestamp): Date to select on
        strategy (str): Strategy type:
            - 'contrarian': Long high DW (mean reversion), short low DW (momentum exhaustion)
            - 'momentum_continuation': Long low DW (trending), short high DW (choppy)
            - 'sweet_spot': Long moderate DW, short extreme DW
            - 'long_high_dw': Long high DW only (mean reversion)
        num_quintiles (int): Number of quintiles
    ...
    if strategy == 'contrarian':
        # Long high DW (mean reverting), short low DW (momentum)
        ...
        # Long low DW (trending), short high DW (choppy)
        ...
        # Long moderate DW (1.5-2.0), short extreme DW
        # Define moderate as 40th-60th percentile
        long_df = ranked_df[(ranked_df['percentile'] >= 40) & (ranked_df['percentile'] <= 60)]
        # Extreme as below 20th or above 80th percentile
        short_df = ranked_df[(ranked_df['percentile'] <= 20) | (ranked_df['percentile'] >= 80)]
        
    elif strategy == 'long_high_dw':
...
def calculate_position_weights(positions_df, weighting_method='equal_weight', total_allocation=0.5):
    ...
    Args:
        ...
        weighting_method (str): Weighting method ('equal_weight', 'risk_parity', 'dw_weighted')
    ...
    elif weighting_method == 'dw_weighted':
        # Weight by distance from DW=2 (neutral)
        if 'dw' not in df.columns or df['dw'].isna().all():
            df['weight'] = total_allocation / len(df)
        else:
            df['dw_clean'] = df['dw'].fillna(2.0)
            df['dw_distance'] = np.abs(df['dw_clean'] - 2.0)
            
            # Avoid division by zero
            if df['dw_distance'].sum() == 0:
                df['weight'] = total_allocation / len(df)
            else:
                df['weight'] = (df['dw_distance'] / df['dw_distance'].sum()) * total_allocation
...
def run_backtest(data, strategy='contrarian', dw_window=30, dw_method='raw_returns',
    ...
    Args:
        ...
        dw_method (str): DW calculation method ('raw_returns' or 'residuals')
    ...
    """
    print("=" * 80)
    print("DURBIN-WATSON FACTOR BACKTEST")
    print("=" * 80)
    print(f"\nStrategy: {strategy}")
    print(f"DW Window: {dw_window} days")
    print(f"DW Method: {dw_method}")
    ...
    # Step 1: Calculate Durbin-Watson statistic
    print("\n" + "-" * 80)
    print("Step 1: Calculating rolling Durbin-Watson statistic...")
    dw_data = calculate_rolling_dw(data, window=dw_window, method=dw_method)
    print(f"  Total data points with DW: {dw_data['dw'].notna().sum()}")
    valid_dw = dw_data['dw'].dropna()
    if len(valid_dw) > 0:
        print(f"  DW range: [{valid_dw.min():.2f}, {valid_dw.max():.2f}]")
        print(f"  DW mean: {valid_dw.mean():.2f}")
        print(f"  DW median: {valid_dw.median():.2f}")
    
    # Step 2: Calculate volatility (for risk parity weighting)
    print("\n" + "-" * 80)
    print("Step 2: Calculating volatility...")
    dw_data = calculate_volatility(dw_data, window=volatility_window)
    ...
    if start_date:
        dw_data = dw_data[dw_data['date'] >= pd.to_datetime(start_date)]
    if end_date:
        dw_data = dw_data[dw_data['date'] <= pd.to_datetime(end_date)]
    ...
    # Get trading dates
    trading_dates = sorted(dw_data['date'].unique())
    ...
    for date_idx, current_date in enumerate(trading_dates):
        ...
        if current_date in rebalance_dates:
            # Select positions based on DW
            selected = select_symbols_by_dw(
                dw_data, current_date, strategy=strategy,
            ...
            for _, row in long_positions.iterrows():
                trades.append({
                    ...
                    'autocorr_30d': row.get('autocorr_30d', np.nan),
        ...
        if date_idx < len(trading_dates) - 1:
            ...
            next_day_data = dw_data[dw_data['date'] == next_date]
        ...
        # Calculate portfolio average DW
        portfolio_avg_dw = 0
        for symbol, weight in current_positions.items():
            symbol_dw_data = dw_data[(dw_data['date'] == current_date) & 
                                    (dw_data['symbol'] == symbol)]
            if not symbol_dw_data.empty:
                dw_val = symbol_dw_data.iloc[0]['dw']
                if not np.isnan(dw_val):
                    portfolio_avg_dw += abs(weight) * dw_val
    ...
    strategy_info = {
        ...
        'avg_long_dw': trades_df[trades_df['signal'] == 'LONG']['dw'].mean() if not trades_df.empty else np.nan,
        'avg_short_dw': trades_df[trades_df['signal'] == 'SHORT']['dw'].mean() if not trades_df.empty else np.nan,
...
def calculate_metrics(portfolio_df, initial_capital, leverage):
    ...
    # Average DW
    avg_portfolio_dw = portfolio_df['avg_dw'].mean()
    ...
    metrics = {
        ...
        'avg_portfolio_dw': avg_portfolio_dw,
    }
...
def print_results(results):
    ...
    print(f"  DW Method:              {strategy_info['dw_method']}")
    ...
    print(f"  Avg Portfolio DW:       {metrics['avg_portfolio_dw']:>14.2f}")
    ...
    print("\nDW Analysis:")
    print(f"  Avg Long DW:            {strategy_info['avg_long_dw']:>14.2f}")
    print(f"  Avg Short DW:           {strategy_info['avg_short_dw']:>14.2f}")
...
def main():
    """Main function to run Durbin-Watson factor backtest."""
    parser = argparse.ArgumentParser(description='Backtest Durbin-Watson factor strategy')
    ...
    # Strategy parameters
    parser.add_argument('--strategy', type=str, default='contrarian',
                       choices=['contrarian', 'momentum_continuation', 'sweet_spot', 'long_high_dw'],
                       help='Strategy variant')
    
    # DW calculation parameters
    parser.add_argument('--dw-window', type=int, default=30,
                       help='DW calculation window in days')
    parser.add_argument('--dw-method', type=str, default='raw_returns',
                       choices=['raw_returns', 'residuals'],
                       help='DW calculation method')
    ...
    parser.add_argument('--weighting-method', type=str, default='equal_weight',
                       choices=['equal_weight', 'risk_parity', 'dw_weighted'],
    ...
    parser.add_argument('--output-prefix', type=str,
                       default='backtests/results/backtest_durbin_watson',
