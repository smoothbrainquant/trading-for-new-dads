...
1. Calculates rolling skewness of daily returns for each cryptocurrency
2. Ranks cryptocurrencies by skewness values
3. Creates long/short portfolios based on skewness rankings:
   - Mean Reversion: Long low skew (symmetric), Short high skew (right-skewed)
   - Momentum: Long high skew (right-skewed), Short low skew (symmetric/left-skewed)
...
Skewness hypothesis: Skewness measures asymmetry of return distributions.
Positive skew = more extreme positive moves, negative skew = more extreme negative moves
...
def calculate_rolling_skew(data, window=30):
    """
    Calculate rolling skewness of daily returns.
    Uses Fisher-Pearson standardized moment coefficient.
    
    Args:
        data (pd.DataFrame): DataFrame with date, symbol, close columns
        window (int): Rolling window size for skewness calculation
        
    Returns:
        pd.DataFrame: DataFrame with skewness and supporting columns
    """
    df = data.copy()
    df = df.sort_values(['symbol', 'date']).reset_index(drop=True)
    ...
    def rolling_skewness(x):
        if len(x) < window or x.isna().all():
            return np.nan
        try:
            # bias=False: unbiased estimator (sample skewness)
            # nan_policy='omit' to handle any NaN values
            return stats.skew(x.dropna(), bias=False, nan_policy='omit')
        except:
            return np.nan
    
    df['skewness'] = df.groupby('symbol')['daily_return'].transform(
        lambda x: x.rolling(window=window, min_periods=window).apply(rolling_skewness, raw=False)
    )
...
def rank_by_skewness(data, current_date):
    """
    Rank symbols by skewness on a given date.
    
    Args:
        data (pd.DataFrame): DataFrame with skewness values
    ...
    # Remove rows with missing skewness
    date_data = date_data.dropna(subset=['skewness'])
    
    if len(date_data) == 0:
        return pd.DataFrame()
    
    # Rank by skewness (1 = lowest skewness, N = highest skewness)
    date_data['skewness_rank'] = date_data['skewness'].rank(method='average')
    date_data['skewness_percentile'] = date_data['skewness_rank'] / len(date_data) * 100
    
    return date_data


def select_symbols_by_skewness(ranked_data, strategy='mean_reversion', 
                                long_percentile=20, short_percentile=80,
                                max_positions=10):
    """
    Select symbols for long/short based on skewness rankings.
    
    Args:
        ranked_data (pd.DataFrame): Ranked data with skewness_percentile column
        strategy (str): Strategy type:
            - 'mean_reversion': Long low skew, Short high skew
            - 'momentum': Long high skew, Short low skew
            - 'long_only_symmetric': Long low skew only
            - 'long_only_positive_skew': Long high skew only
    ...
    if strategy == 'mean_reversion':
        # Long: Low skewness (symmetric distributions)
        long_data = ranked_data[ranked_data['skewness_percentile'] <= long_percentile]
        # Short: High skewness (right-skewed distributions)
        short_data = ranked_data[ranked_data['skewness_percentile'] >= short_percentile]
        
    elif strategy == 'momentum':
        # Long: High skewness (right-skewed distributions)
        long_data = ranked_data[ranked_data['skewness_percentile'] >= short_percentile]
        # Short: Low skewness (left-skewed or symmetric distributions)
        short_data = ranked_data[ranked_data['skewness_percentile'] <= long_percentile]
        
    elif strategy == 'long_only_symmetric':
        # Long: Low skewness only (symmetric distributions)
        long_data = ranked_data[ranked_data['skewness_percentile'] <= long_percentile]
        short_data = pd.DataFrame()
        
    elif strategy == 'long_only_positive_skew':
        # Long: High skewness only (right-skewed distributions)
        long_data = ranked_data[ranked_data['skewness_percentile'] >= short_percentile]
        short_data = pd.DataFrame()
    ...
    if len(long_data) > max_positions:
        # For mean reversion: take lowest skewness
        # For momentum: take highest skewness
        if strategy in ['mean_reversion', 'long_only_symmetric']:
            long_data = long_data.nsmallest(max_positions, 'skewness')
        else:
            long_data = long_data.nlargest(max_positions, 'skewness')
    
    if len(short_data) > max_positions:
        # For mean reversion: take highest skewness
        # For momentum: take lowest skewness
        if strategy == 'mean_reversion':
            short_data = short_data.nlargest(max_positions, 'skewness')
        else:
            short_data = short_data.nsmallest(max_positions, 'skewness')
...
def backtest(
    ...
    """
    print("\nStep 1: Calculating returns and skewness...")
    data_with_skewness = calculate_rolling_skew(price_data, window=skewness_window)
    
    print("Step 2: Calculating volatility...")
    data_with_volatility = calculate_volatility(data_with_skewness, window=volatility_window)
    ...
    # Need enough data for skewness calculation
    min_required_days = max(skewness_window, volatility_window) + 10
    ...
    backtest_start_idx = max(skewness_window, volatility_window)
    ...
    print(f"  Skewness window: {skewness_window}d")
    ...
    for date_idx, current_date in enumerate(daily_tracking_dates):
        ...
        if is_rebalance_day:
            ...
            # Rank by skewness
            ranked_data = rank_by_skewness(historical_data, current_date)
            
            if not ranked_data.empty:
                # Select long/short symbols
                selected = select_symbols_by_skewness(
                    ranked_data,
                    strategy=strategy,
                ...
                # Save skewness data for analysis
                for _, row in selected['long'].iterrows():
                    skewness_timeseries.append({
                        'date': current_date,
                        'symbol': row['symbol'],
                        'signal': 'LONG',
                        'skewness': row['skewness'],
                        'skewness_rank': row['skewness_rank'],
                        'skewness_percentile': row['skewness_percentile'],
                ...
                if new_weights != current_weights:
                    ...
                    for symbol in all_symbols:
                        ...
                        if abs(new_weight - old_weight) > 0.0001:
                            # Get skewness for this symbol
                            skew = ranked_data[ranked_data['symbol'] == symbol]['skewness'].values
                            skew_val = skew[0] if len(skew) > 0 else np.nan
    ...
    skewness_df = pd.DataFrame(skewness_timeseries)
    ...
    return {
        ...
        'trades': trades_df,
        'skewness_timeseries': skewness_df,
        'metrics': metrics,
        'strategy_info': {
            'strategy': strategy,
...
def print_results(results):
    ...
    print("\n" + "=" * 80)
    print("SKEWNESS FACTOR BACKTEST RESULTS")
    print("=" * 80)
    
    print(f"\nStrategy Information:")
    print(f"  Strategy: {strategy_info['strategy']}")
    print(f"  Skewness window: {strategy_info['skewness_window']}d")
...
def save_results(results, output_prefix='backtests/results/skew_factor'):
    ...
    if not results['skewness_timeseries'].empty:
        skewness_file = f"{output_prefix}_skewness_timeseries.csv"
        results['skewness_timeseries'].to_csv(skewness_file, index=False)
        print(f"Skewness timeseries saved to: {skewness_file}")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Backtest Skewness Factor Trading Strategy',
    ...
    parser.add_argument('--strategy', type=str, default='mean_reversion',
                       choices=['mean_reversion', 'momentum', 'long_only_symmetric', 'long_only_positive_skew'],
                       help='Strategy type')
    parser.add_argument('--skewness-window', type=int, default=30,
                       help='Window for skewness calculation in days')
    ...
    print("=" * 80)
    print("SKEWNESS FACTOR BACKTEST")
    print("=" * 80)
    ...
    results = backtest(
        ...
        skewness_window=args.skewness_window,
