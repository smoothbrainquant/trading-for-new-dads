...
def calculate_skewness(data, lookback_window=30, min_volume=5_000_000, min_market_cap=50_000_000):
    ...
    df["returns"] = df.groupby("symbol")["close"].transform(lambda x: np.log(x / x.shift(1)))
    ...
    df["passes_all_filters"] = (
        df["passes_volume_filter"] & df["passes_marketcap_filter"] & df["passes_data_filter"]
    )
...
def backtest_strategy(
    signals_df, start_date=None, end_date=None, initial_capital=10000, strategy_type="long_short"
):
    """
    ...
    for i, current_date in enumerate(all_dates):
        ...
        if (i + 1) % 100 == 0 or i == len(all_dates) - 1:
            print(
                f"Progress: {i+1}/{len(all_dates)} days | "
    ...
    return {"portfolio_values": portfolio_df, "metrics": metrics, "signals_df": signals_df}
...
def calculate_performance_metrics(portfolio_df, initial_capital):
    ...
    downside_vol = (
        negative_returns.std() * np.sqrt(365) if len(negative_returns) > 0 else annualized_vol
    )
    ...
    portfolio_df["drawdown"] = (
        portfolio_df["cumulative"] - portfolio_df["running_max"]
    ...
    long_sharpe = (
        (long_returns.mean() / long_returns.std() * np.sqrt(365))
        if len(long_returns) > 0 and long_returns.std() > 0
        else 0
    )
    short_sharpe = (
        (short_returns.mean() / short_returns.std() * np.sqrt(365))
        if len(short_returns) > 0 and short_returns.std() > 0
        else 0
    )
...
def create_visualizations(results, output_dir="backtests/results"):
    ...
    fig, ax = plt.subplots(figsize=(12, 6))
    ax.fill_between(
        portfolio_df["date"], portfolio_df["drawdown"] * 100, 0, color="#A23B72", alpha=0.6
    )
    ax.set_xlabel("Date", fontsize=12)
    ...
    fig, ax = plt.subplots(figsize=(12, 6))
    ax.hist(
        signals_with_skew["skewness_30d"], bins=50, color="#F18F01", alpha=0.7, edgecolor="black"
    )
...
def save_results(results, output_prefix="backtests/results/skew_factor"):
    ...
    signals_subset = results["signals_df"][
        results["signals_df"]["date"].isin(portfolio_dates) & (results["signals_df"]["signal"] != 0)
    ]
...
def main():
    ...
    parser.add_argument(
        "--start-date", type=str, default="2020-03-01", help="Start date for backtest (YYYY-MM-DD)"
    )
    parser.add_argument(
    ...
    signals_df = generate_signals(
        data_with_skewness, num_quintiles=args.num_quintiles, strategy_type=args.strategy_type
    )
