...
def simulate_regime_switching(
    portfolios, btc_data, initial_capital=10000, switching_mode="optimal"
):
    """
    ...
    for idx, row in combined.iterrows():
        if switching_mode == "optimal":
            ...
            daily_return = tf_weight * row["tf_daily_return"] + mr_weight * row["mr_daily_return"]
...
def main():
    ...
    print(
        f"Date range: {portfolios['trend_following']['date'].min().date()} to {portfolios['trend_following']['date'].max().date()}"
    )
    ...
    results["optimal"] = {"portfolio": optimal_portfolio, "metrics": optimal_metrics}
    ...
    results["blended"] = {"portfolio": blended_portfolio, "metrics": blended_metrics}
    print_results(results["blended"], "Blended Switching (80/20)")

    # Save results
    optimal_portfolio.to_csv(
        "backtests/results/adf_regime_switching_optimal_portfolio.csv", index=False
    )
    blended_portfolio.to_csv(
        "backtests/results/adf_regime_switching_blended_portfolio.csv", index=False
    )
    ...
    print(
        "✓ Saved blended switching to: backtests/results/adf_regime_switching_blended_portfolio.csv"
    )
    ...
    tf_metrics = pd.read_csv(
        "backtests/results/adf_trend_following_2021_top100_metrics.csv", index_col=0
    )
    mr_metrics = pd.read_csv(
        "backtests/results/adf_mean_reversion_2021_top100_metrics.csv", index_col=0
    )

    comparison = pd.DataFrame(
        {
            "Strategy": [
                "Trend Following (Static)",
    ...
    print(
        f"  Improvement:                 {optimal_metrics['annualized_return'] - tf_return:>+8.2%} (+{(optimal_metrics['annualized_return'] / tf_return - 1)*100:.1f}%)"
    )
    ...
    print(
        f"  Improvement:                 {blended_metrics['annualized_return'] - tf_return:>+8.2%} (+{(blended_metrics['annualized_return'] / tf_return - 1)*100:.1f}%)"
    )
    ...
    print(
        f"  Optimal Switching:           {optimal_metrics['sharpe_ratio']:>8.2f} (+{optimal_metrics['sharpe_ratio'] - tf_sharpe:.2f})"
    )
    print(
        f"  Blended Switching:           {blended_metrics['sharpe_ratio']:>8.2f} (+{blended_metrics['sharpe_ratio'] - tf_sharpe:.2f})"
    )
