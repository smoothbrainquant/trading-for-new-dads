#!/usr/bin/env python3
"""
Dilution Factor Decile Analysis - RISK PARITY WEIGHTED

This script runs comprehensive dilution factor backtests with:
1. Individual top 10 and bottom 10 coin performance
2. Decile portfolio performance with RISK PARITY weighting (inverse volatility)
3. Detailed comparison vs equal-weighted approach

Strategy:
- Universe: Top 150 coins by market cap with dilution data
- Sort by dilution velocity (12-month rolling)
- Create 10 risk-parity weighted decile portfolios
- Monthly rebalancing
...
def backtest_risk_parity_deciles(signals_df, price_df, n_deciles=10):
    """
    Backtest portfolios sorted into deciles with RISK PARITY weighting.
    ...
    for idx, rebal_date in enumerate(rebalance_dates):
        ...
        # Calculate volatility for all coins
        date_signals['volatility'] = date_signals['symbol'].apply(
            lambda s: calculate_volatility(price_df, s, rebal_date, lookback_days=90)
        )
        
        # Filter to coins with valid volatility
        date_signals = date_signals[date_signals['volatility'].notna()]
        date_signals = date_signals[date_signals['volatility'] > 0]
        
        if len(date_signals) < 20:
            print(f"  Warning: Only {len(date_signals)} coins with valid volatility, skipping")
            continue
        ...
        # Calculate risk parity weights and returns for each decile
        for decile in range(1, n_deciles + 1):
            decile_coins = date_signals[date_signals['decile'] == decile].copy()
            
            if len(decile_coins) == 0:
                continue
            
            # Calculate inverse volatility weights (risk parity)
            decile_coins['inv_vol'] = 1.0 / decile_coins['volatility']
            total_inv_vol = decile_coins['inv_vol'].sum()
            decile_coins['weight'] = decile_coins['inv_vol'] / total_inv_vol
            
            # Store holdings and weights for this period
            ...
            # Store average weight statistics
            decile_results[decile]['weights_history'].append({
                'date': rebal_date,
                'n_coins': len(decile_coins),
                'avg_weight': decile_coins['weight'].mean(),
                'max_weight': decile_coins['weight'].max(),
                'min_weight': decile_coins['weight'].min(),
                'weight_concentration': (decile_coins['weight'] ** 2).sum()  # HHI
            })
            ...
            for date in sorted(holding_period_returns['date'].unique()):
                ...
                # Get weighted returns for coins in this decile
                portfolio_return = 0.0
                total_weight = 0.0
                
                for _, coin in decile_coins.iterrows():
                    coin_return = daily_data[daily_data['base'] == coin['symbol']]['return'].values
                    if len(coin_return) > 0 and not np.isnan(coin_return[0]):
                        portfolio_return += coin['weight'] * coin_return[0]
                        total_weight += coin['weight']
                
                # Normalize if we have valid returns
                if total_weight > 0:
        ...
        # Track individual coin returns for this period
    ...
    for decile in range(1, n_deciles + 1):
        ...
        decile_results[decile]['weights_history'] = pd.DataFrame(
            decile_results[decile]['weights_history']
        )
...
def calculate_decile_metrics(decile_results):
    ...
    for decile, results in decile_results.items():
        ...
        # Average weight concentration (from weights history)
        weights_df = results['weights_history']
        avg_concentration = weights_df['weight_concentration'].mean() if len(weights_df) > 0 else np.nan
        avg_coins = weights_df['n_coins'].mean() if len(weights_df) > 0 else np.nan
        ...
        metrics[decile] = {
            ...
            'avg_coins': avg_coins,
            'avg_weight_concentration': avg_concentration
        }
...
def analyze_top_bottom_names(individual_returns_df, n=10):
    ...
    coin_avg_returns.columns = ['symbol', 'avg_period_return', 'avg_dilution_velocity', 
                                 'avg_rank', 'avg_volatility', 'typical_decile', 'n_periods']
...
def plot_risk_parity_comparison(decile_results, metrics_df, output_file='dilution_decile_risk_parity_comparison.png'):
    """Plot comprehensive risk parity decile comparison."""
    ...
    ax1.set_title('Decile Portfolio Performance (Risk Parity Weighted)', fontsize=12, fontweight='bold')
    ...
    # Plot 4: Weight concentration (HHI)
    ax4 = axes[1, 1]
    if 'avg_weight_concentration' in metrics_df.columns:
        colors = ['darkgreen' if i == 1 else 'darkred' if i == 10 else 'gray' 
                  for i in metrics_df['decile']]
        bars = ax4.bar(metrics_df['decile'], metrics_df['avg_weight_concentration'], color=colors, alpha=0.7)
        ax4.set_xlabel('Decile (1=Low Dilution, 10=High Dilution)', fontsize=11)
        ax4.set_ylabel('Weight Concentration (HHI)', fontsize=11)
        ax4.set_title('Portfolio Concentration by Decile', fontsize=12, fontweight='bold')
...
def main():
    ...
    print("=" * 80)
    print("DILUTION FACTOR DECILE ANALYSIS - RISK PARITY WEIGHTED")
    print("=" * 80)
    print("\nAnalysis:")
    ...
    print("  - 90-day volatility lookback for weights")
    print()
    ...
    # Run risk parity decile backtest
    print("\n" + "-" * 80)
    print("RUNNING RISK PARITY DECILE BACKTEST")
    print("-" * 80)
    
    decile_results, individual_returns_df = backtest_risk_parity_deciles(signals_df, price_df, n_deciles=10)
    ...
    # Print results
    print("\n" + "=" * 80)
    print("RISK PARITY WEIGHTED - DECILE PERFORMANCE SUMMARY")
    print("=" * 80)
    print()
    ...
    print(top_names[['symbol', 'avg_period_return', 'avg_dilution_velocity', 
                     'avg_volatility', 'typical_decile', 'n_periods']].to_string(index=False))
    ...
    if len(metrics_df) >= 2:
        ...
        print("\n" + "=" * 80)
        print("RISK PARITY: DECILE 1 vs DECILE 10")
        print("=" * 80)
        ...
        print(f"  Volatility:         {d1['volatility_pct']:>8.2f}%")
        ...
        print(f"  Volatility:         {d10['volatility_pct']:>8.2f}%")
        ...
        print(f"  Vol Difference:     {d1['volatility_pct'] - d10['volatility_pct']:>8.2f}%")
    ...
    metrics_df.to_csv('dilution_decile_risk_parity_metrics.csv', index=False)
    print("✓ Saved: dilution_decile_risk_parity_metrics.csv")
    
    top_names.to_csv('dilution_risk_parity_top10_names.csv', index=False)
    print("✓ Saved: dilution_risk_parity_top10_names.csv")
    
    bottom_names.to_csv('dilution_risk_parity_bottom10_names.csv', index=False)
    print("✓ Saved: dilution_risk_parity_bottom10_names.csv")
    
    individual_returns_df.to_csv('dilution_risk_parity_individual_returns.csv', index=False)
    print("✓ Saved: dilution_risk_parity_individual_returns.csv")
    
    # Save decile portfolio values
    for decile, results in decile_results.items():
        if len(results['portfolio_values']) > 0:
            results['portfolio_values'].to_csv(f'dilution_rp_decile{decile}_portfolio.csv', index=False)
        if len(results['weights_history']) > 0:
            results['weights_history'].to_csv(f'dilution_rp_decile{decile}_weights.csv', index=False)
    print("✓ Saved: dilution_rp_decile*_portfolio.csv (10 files)")
    print("✓ Saved: dilution_rp_decile*_weights.csv (10 files)")
    ...
    plot_risk_parity_comparison(decile_results, metrics_df)
    
    print("\n" + "=" * 80)
    print("RISK PARITY ANALYSIS COMPLETE")
    print("=" * 80)
