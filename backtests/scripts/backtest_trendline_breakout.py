#!/usr/bin/env python3
"""
Backtest for Trendline Breakout Strategy

This script backtests a trendline breakout strategy that:
1. Calculates rolling linear regression on closing prices (trendline)
2. Measures how far the current price is from the trendline
3. Detects breakouts when price closes strongly above/below the trendline
4. Only trades when the trendline is clean and strong (high R²)
5. Long breakouts above uptrend, short breakouts below downtrend
6. Uses next-day returns to avoid lookahead bias

Hypothesis: Clean trendlines with strong breakouts signal continuation.
"""
...
def calculate_rolling_trendline(data, window=30):
    ...
    def fit_trendline(prices, current_idx):
        """Fit linear regression to price window and predict current price"""
        n = len(prices)
...
def calculate_breakout_metrics(df, window=30):
    ...
    Args:
        df (pd.DataFrame): DataFrame with close, predicted_price, r_squared
    ...
    # Normalize distance by volatility (Z-score of distance)
    # This tells us how many standard deviations the price is from the trendline
...
def select_top_signals(data, date, max_positions=10, side='both'):
    """
    Select top breakout signals for a given date.
    
    Args:
        data (pd.DataFrame): Data with signal and signal_strength columns
        date (pd.Timestamp): Date to select signals on
...
def calculate_position_weights(positions_df, weighting_method='equal_weight', total_allocation=0.5):
    ...
    Args:
        ...
        weighting_method (str): 'equal_weight', 'signal_weighted', or 'risk_parity'
        total_allocation (float): Total allocation to this side
        
    Returns:
        pd.DataFrame: DataFrame with weights column
    """
...
def run_backtest(data,
    ...
    Args:
        ...
        volatility_window (int): Volatility window for breakout normalization
        breakout_threshold (float): Z-score threshold for breakout
        min_r2 (float): Minimum R² for clean trendline
        max_pvalue (float): Maximum p-value for significant trendline
        slope_direction (str): 'positive', 'negative', or 'any'
        rebalance_days (int): How often to check for new signals
        max_positions (int): Maximum positions per side
        holding_period (int): Days to hold each signal
    ...
    Returns:
        dict: Dictionary with portfolio_values, trades, metrics, strategy_info
    """
    print("=" * 80)
    print("TRENDLINE BREAKOUT BACKTEST")
    print("=" * 80)
    ...
    print(f"Slope Direction: {slope_direction}")
    ...
    print(f"  Total data points: {len(trendline_data)}")
    ...
    print(f"  Distance from trendline range: [{trendline_data['distance_from_trendline'].min():.2f}%, {trendline_data['distance_from_trendline'].max():.2f}%]")
    print(f"  Breakout Z-score range: [{trendline_data['breakout_z_score'].min():.2f}, {trendline_data['breakout_z_score'].max():.2f}]")
            ...
            for _, row in long_positions.iterrows():
                if row['symbol'] not in current_positions:
                    ...
                    trades.append({
                        ...
                        'breakout_z_score': row['breakout_z_score'],
...
def calculate_metrics(portfolio_df, trades_df, initial_capital, leverage):
    ...
    num_long_trades = len(trades_df[(trades_df['action'] == 'OPEN') & (trades_df['signal'] == 'LONG')]) if not trades_df.empty else 0
    num_short_trades = len(trades_df[(trades_df['action'] == 'OPEN') & (trades_df['signal'] == 'SHORT')]) if not trades_df.empty else 0
    ...
    avg_breakout_z = open_trades['breakout_z_score'].mean() if not open_trades.empty else np.nan
    ...
    metrics = {
        ...
        'avg_r2': avg_r2,
        'avg_breakout_z_score': avg_breakout_z,
...
def print_results(results):
    ...
    print(f"  Breakout Threshold:     {strategy_info['breakout_threshold']} σ")
    print(f"  Min R²:                 {strategy_info['min_r2']}")
    print(f"  Max P-value:            {strategy_info['max_pvalue']}")
    print(f"  Slope Direction:        {strategy_info['slope_direction']}")
    ...
    print(f"  Long Trades:            {metrics['num_long_trades']:>14.0f}")
    print(f"  Short Trades:           {metrics['num_short_trades']:>14.0f}")
    print(f"  Avg Positions:          {metrics['avg_positions']:>14.1f}")
    
    print("\nSignal Quality:")
    print(f"  Avg R²:                 {metrics['avg_r2']:>14.4f}")
    print(f"  Avg Breakout Z-score:   {metrics['avg_breakout_z_score']:>14.2f}")
...
def main():
    """Main function."""
    parser = argparse.ArgumentParser(description='Backtest trendline breakout strategy')
    ...
    parser.add_argument('--output-prefix', type=str,
                       default='backtests/results/backtest_trendline_breakout',
