...
def analyze_by_regime(portfolio_df, strategy_name):
    ...
    for regime in ["Strong Up (>10%)", "Up (0-10%)", "Down (0 to -10%)", "Strong Down (<-10%)"]:
        ...
        total_return = (
            regime_data["portfolio_value"].iloc[-1] / regime_data["portfolio_value"].iloc[0] - 1
        ) * 100
...
def main():
    ...
    for regime in ["Strong Up (>10%)", "Up (0-10%)", "Down (0 to -10%)", "Strong Down (<-10%)"]:
        ...
        if len(regime_data) > 0:
            print(f"\n{regime}:")
            print(
                regime_data[["Strategy", "Ann_Return", "Sharpe", "Win_Rate"]].to_string(index=False)
            )
    ...
    for strategy_name in strategies.keys():
        ...
        if len(strategy_data) > 0:
            ...
            print(f"\n{strategy_name}:")
            print(
                f"  Best:  {strategy_data.iloc[0]['Regime']:25s} ({strategy_data.iloc[0]['Ann_Return']:+7.2f}% ann. return)"
            )
            print(
                f"  Worst: {strategy_data.iloc[-1]['Regime']:25s} ({strategy_data.iloc[-1]['Ann_Return']:+7.2f}% ann. return)"
            )
