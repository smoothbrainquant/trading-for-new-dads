...
def select_symbols_by_adf(data, date, strategy='mean_reversion_premium', 
...
def run_backtest(data, strategy='mean_reversion_premium', adf_window=60, 
                volatility_window=30, rebalance_days=7, num_quintiles=5,
                long_percentile=20, short_percentile=80, regression='ct',
    ...
    print(f"  Stationary coins (p<0.05): {adf_data['is_stationary'].sum()} / {len(adf_data)} ({adf_data['is_stationary'].mean()*100:.1f}%)")
    ...
    for date_idx, current_date in enumerate(trading_dates):
        ...
        if current_date in rebalance_dates:
            ...
            selected = select_symbols_by_adf(
                adf_data, current_date, strategy=strategy,
        ...
        for symbol, weight in current_positions.items():
            symbol_adf_data = adf_data[(adf_data['date'] == current_date) & 
                                      (adf_data['symbol'] == symbol)]
    ...
    strategy_info = {
        ...
        'avg_adf_long': trades_df[trades_df['signal'] == 'LONG']['adf_stat'].mean() if not trades_df.empty else np.nan,
        'avg_adf_short': trades_df[trades_df['signal'] == 'SHORT']['adf_stat'].mean() if not trades_df.empty else np.nan,
...
def main():
    ...
    # Strategy parameters
    parser.add_argument('--strategy', type=str, default='mean_reversion_premium',
                       choices=['mean_reversion_premium', 'trend_following_premium',
                               'long_stationary', 'long_trending'],
                       help='Strategy variant')
    
    # ADF calculation parameters
    parser.add_argument('--adf-window', type=int, default=60,
                       help='ADF calculation window in days')
    parser.add_argument('--regression', type=str, default='ct',
                       choices=['c', 'ct', 'ctt', 'n'],
