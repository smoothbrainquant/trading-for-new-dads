...
def backtest(data, days_threshold=20, lookback_window=200, rebalance_freq='daily', 
             start_date=None, end_date=None, initial_capital=10000, volatility_window=30):
    """
    ...
    if len(all_dates) < min_required_days:
        ...
        print(f"WARNING: Insufficient data for {lookback_window}d lookback + {volatility_window}d volatility.")
        print(f"         Available days: {len(all_dates)}")
        print(f"         Adjusting to {adjusted_lookback}d lookback + {adjusted_volatility}d volatility")
    ...
    for i, current_date in enumerate(backtest_dates):
        ...
        try:
            ...
            latest_days_from_high = days_from_high_df[
                days_from_high_df['date'] == current_date
            ]
        ...
        if len(selected_symbols) > 0:
            ...
            try:
                volatility_df = calculate_rolling_volatility_custom(selected_data, window=volatility_window)
