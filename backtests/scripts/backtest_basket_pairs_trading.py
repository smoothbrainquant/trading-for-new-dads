    ...
    def update_equity(self, date: pd.Timestamp, price_data: pd.DataFrame):
        ...
        for pos in self.open_positions:
            price_row = price_data[
                (price_data["base"] == pos.symbol) & (price_data["date"] == date)
            ]
...
def calculate_position_sizes(
    signals: pd.DataFrame, portfolio: PortfolioTracker, config: BacktestConfig
) -> Dict[str, float]:
    """
        ...
        if not long_signals.empty:
            ...
            if total_z > 0:
                for _, row in long_signals.iterrows():
                    ...
                    position_sizes[row["symbol"]] = min(
                        weight * target_capital, target_capital * 0.15
                    )
...
def check_exit_conditions(
    position: Position, current_data: pd.Series, config: BacktestConfig
) -> Tuple[bool, str]:
    """
...
def run_backtest(config: BacktestConfig) -> Tuple[PortfolioTracker, pd.DataFrame]:
    ...
    for i, date in enumerate(trading_dates):
        ...
        for pos in portfolio.open_positions:
            ...
            pos_signal = day_signals[
                (day_signals["symbol"] == pos.symbol) & (day_signals["category"] == pos.category)
            ]
        ...
        if not new_signals.empty:
            ...
            for _, signal in new_signals.iterrows():
                ...
                if not portfolio.can_add_position(
                    category, config.max_positions_per_category, config.max_total_positions
                ):
                    continue
        ...
        if not next_prices.empty:
            portfolio.update_equity(next_date if i < len(trading_dates) - 1 else date, next_prices)
...
def calculate_performance_metrics(portfolio: PortfolioTracker) -> pd.Series:
    ...
    avg_hold = (
        np.mean([p.hold_days for p in portfolio.closed_positions])
        if portfolio.closed_positions
        else 0
    )
...
def plot_equity_curve(portfolio: PortfolioTracker, output_path: str):
    ...
    axes[0].axhline(
        y=portfolio.initial_capital, color="gray", linestyle="--", label="Initial Capital"
    )
...
def plot_category_performance(trades_df: pd.DataFrame, output_path: str):
    ...
    # Group by category
    category_perf = (
        trades_df.groupby("category")
        .agg({"pnl": "sum", "return_pct": "mean", "symbol": "count"})
...
def save_results(
    ...
    summary_df = pd.DataFrame({"metric": metrics.index, "value": metrics.values})
...
def main():
    parser = argparse.ArgumentParser(description="Backtest basket pairs trading strategy")
    parser.add_argument(
        "--signal-file",
        type=str,
    ...
    parser.add_argument(
        "--holding-period", type=int, default=10, help="Days to hold positions (default: 10)"
    )
    parser.add_argument(
        "--stop-loss", type=float, default=0.10, help="Stop loss threshold (default: 0.10 = 10%%)"
    )
    parser.add_argument(
        "--take-profit",
        type=float,
    ...
    parser.add_argument("--max-positions", type=int, default=20, help="Maximum total positions")
    parser.add_argument(
        "--categories", type=str, nargs="+", help="Categories to trade (default: all)"
    )
    ...
    parser.add_argument(
        "--output-dir", type=str, default="backtests/results", help="Output directory for results"
    )
    ...
    if not trades_df.empty:
        plot_category_performance(
            trades_df, f"{args.output_dir}/pairs_trading_category_heatmap.png"
        )
