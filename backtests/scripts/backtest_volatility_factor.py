...
def select_symbols_by_volatility(volatility_df, date, strategy='long_low_short_high', num_quintiles=5, vol_column='volatility_30d'):
    """
            ...
            if len(long_symbols) + len(short_symbols) > 0:
                try:
                    if weighting_method == 'equal':
                        ...
                        current_vol_data = historical_volatility[historical_volatility['date'] == current_date]
                ...
                for symbol in all_symbols:
                    ...
                    if abs(new_weight - old_weight) > 0.0001:
                        ...
                        if symbol in long_symbols:
                            ...
                            quintile = num_quintiles if 'high' in strategy or strategy == 'long_low_short_high' else 1
...
def save_results(results, output_prefix='backtest_volatility_factor', output_dir='backtests/results'):
    """
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=5,
        help='Number of volatility quintiles for ranking'
    )
    parser.add_argument(
        '--volatility-window',
        type=int,
