...
def load_data(filepath):
    ...
    try:
        ...
        logger.info(f"✓ Data validation passed: {len(df)} rows, {df['symbol'].nunique()} symbols")
...
def categorize_moves(data, return_threshold=1.0, volume_threshold=1.0):
    ...
    conditions = [
        (df["return_zscore"].abs() > return_threshold)
        & (df["volume_zscore"].abs() > volume_threshold),
        (df["return_zscore"].abs() > return_threshold)
        & (df["volume_zscore"].abs() <= volume_threshold),
        (df["return_zscore"].abs() <= return_threshold)
        & (df["volume_zscore"].abs() > volume_threshold),
    ...
    df["category_directional"] = np.select(
        conditions_directional, categories_directional, default="neutral"
    )
...
def analyze_mean_reversion(data):
    ...
    valid_data = data.dropna(
        subset=["return_zscore", "volume_zscore", "forward_1d_return", "category"]
    )
    ...
    for category in valid_data["category"].unique():
        ...
        stats = {
            ...
            "sharpe_ratio": (
                cat_data["forward_1d_return"].mean()
                / cat_data["forward_1d_return"].std()
                * np.sqrt(365)
                if cat_data["forward_1d_return"].std() > 0
                else 0
            ),
        }
    ...
    for bucket in valid_data_copy["return_bucket"].unique():
        ...
        stats = {
            ...
            "positive_days_pct": (bucket_data["forward_1d_return"] > 0).sum()
            / len(bucket_data)
            * 100,
        }
...
def print_results(results):
    ...
    if not results["by_directional_category"].empty:
        ...
        sorted_df = results["by_directional_category"].sort_values(
            "mean_forward_return", ascending=False
        )
    ...
    if not results["by_directional_category"].empty:
        ...
        if not down_high_vol.empty:
            print(
                f"  Down moves + high volume -> next day return: {down_high_vol['mean_forward_return'].values[0]:.4%}"
            )
        if not down_low_vol.empty:
            print(
                f"  Down moves + low volume -> next day return: {down_low_vol['mean_forward_return'].values[0]:.4%}"
            )
        if not up_high_vol.empty:
            print(
                f"  Up moves + high volume -> next day return: {up_high_vol['mean_forward_return'].values[0]:.4%}"
            )
        if not up_low_vol.empty:
            print(
                f"  Up moves + low volume -> next day return: {up_low_vol['mean_forward_return'].values[0]:.4%}"
            )
...
def main():
    ...
    parser.add_argument(
        "--lookback-window", type=int, default=30, help="Lookback window for calculating z-scores"
    )
    parser.add_argument(
        "--return-threshold",
        type=float,
