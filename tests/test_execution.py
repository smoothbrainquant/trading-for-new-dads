...
from execution.ccxt_get_balance import (
    ccxt_get_hyperliquid_balance,
    ccxt_print_balance_summary
)
...
class TestInstrumentSelection(unittest.TestCase):
    ...
    def test_select_instruments_sorted_by_days(self):
        ...
        if len(result) > 1:
            ...
            self.assertTrue(all(days_col.iloc[i] <= days_col.iloc[i+1] 
                              for i in range(len(days_col)-1)))
    ...
    def test_select_instruments_has_required_columns(self):
        ...
        if len(result) > 0:
            required_columns = ['date', 'symbol', 'high', 
                              'rolling_200d_high', 'days_since_200d_high']
