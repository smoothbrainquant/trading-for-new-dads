...
from .exceptions import DataValidationError, DataStaleError, SignalValidationError, RiskLimitError
...
class DataValidator:
    ...
    def validate_ohlcv_dataframe(
        ...
        if not (df["high"] >= df["close"]).all():
            ...
            raise DataValidationError(f"High < Close in {invalid_count} rows")

        # Check OHLC consistency: low <= close
        if not (df["low"] <= df["close"]).all():
            invalid_count = (df["low"] > df["close"]).sum()
            raise DataValidationError(f"Low > Close in {invalid_count} rows")

        # Check for nulls in critical columns
        critical_nulls = df[required_columns].isnull().sum()
        if critical_nulls.any():
            null_cols = critical_nulls[critical_nulls > 0]
            raise DataValidationError(f"Null values in critical columns: {null_cols.to_dict()}")
        ...
        for col in price_cols:
            if (df[col] <= 0).any():
                ...
                raise DataValidationError(f"Non-positive values in {col}: {invalid_count} rows")
        ...
        for col in numeric_cols:
            if np.isinf(df[col]).any():
                ...
                raise DataValidationError(f"Infinite values in {col}: {invalid_count} rows")

    @staticmethod
    def validate_date_range(
        df: pd.DataFrame, max_age_hours: int = 48, date_column: str = "date"
    ) -> None:
    ...
    def validate_symbols_present(
        df: pd.DataFrame, required_symbols: List[str], symbol_column: str = "symbol"
    ) -> None:
...
class SignalValidator:
    ...
    def validate_signals(signals: Dict[str, List[str]]) -> None:
        ...
        if missing_keys:
            raise SignalValidationError(f"Signal dictionary missing required keys: {missing_keys}")

        # Check types
        if not isinstance(signals["longs"], list):
            raise SignalValidationError(f"'longs' must be a list, got {type(signals['longs'])}")

        if not isinstance(signals["shorts"], list):
            raise SignalValidationError(f"'shorts' must be a list, got {type(signals['shorts'])}")
        ...
        if overlap:
            raise SignalValidationError(f"Symbols appear in both longs and shorts: {overlap}")
    ...
    def validate_weights(weights: Dict[str, float], tolerance: float = 1e-6) -> None:
        ...
        if abs(total - 1.0) > tolerance:
            raise SignalValidationError(f"Weights sum to {total:.6f}, expected 1.0 (±{tolerance})")

        # Check for negative weights
        negative_weights = {k: v for k, v in weights.items() if v < 0}
        if negative_weights:
            raise SignalValidationError(f"Negative weights found: {negative_weights}")
...
class RiskValidator:
    ...
    def validate_position_size(
        notional: float, max_notional: float, symbol: str = "position"
    ) -> None:
        ...
        if notional > max_notional:
            raise RiskLimitError(
                limit_type=f"position_size_{symbol}", current_value=notional, max_value=max_notional
            )

    @staticmethod
    def validate_total_exposure(total_exposure: float, max_exposure: float) -> None:
        ...
        if total_exposure > max_exposure:
            raise RiskLimitError(
                limit_type="total_exposure", current_value=total_exposure, max_value=max_exposure
            )

    @staticmethod
    def validate_leverage(leverage: float, max_leverage: float) -> None:
        ...
        if leverage > max_leverage:
            raise RiskLimitError(
                limit_type="leverage", current_value=leverage, max_value=max_leverage
            )
