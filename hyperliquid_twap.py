...
class hyperliquid(Exchange, ImplicitAPI):
    ...
    def create_twap_order_request(self, symbol: str, side: OrderSide, amount: float, minutes: int, params={}) -> dict:
        ...
        :param str symbol: unified market symbol (spot or swap), e.g., 'ETH/USDC:USDC' or 'PURR/USDC'
        :param str side: 'buy' or 'sell'
        :param float amount: total base size to execute across the TWAP
        :param int minutes: TWAP duration in minutes
        :param dict [params]:
            - bool [reduceOnly]: place as reduce-only (default False)
        ...
        vaultAddress = None
        vaultAddress, params = self.handle_option_and_params_2(params, 'createTwapOrder', 'vaultAddress', 'subAccountAddress')
        vaultAddress = self.format_vault_address(vaultAddress)
        nonce = self.milliseconds()

        twap = {
            'a': self.parse_to_int(market['baseId']),   # asset id (spot uses 10000 + index)
            'b': isBuy,                                  # is buy
            's': sz,                                     # total size
            'r': reduceOnly,                             # reduce only
            'm': self.parse_to_int(minutes),             # minutes
            't': 'random' if randomize else 'na',        # schedule type
        }
        ...
        # Include builder fee if already approved (keeps parity with create_orders_request)
        if self.safe_bool(self.options, 'approvedBuilderFee', False):
            wallet = self.safe_string_lower(self.options, 'builder', '0x6530512A6c89C7cfCEbC3BA7fcD9aDa5f30827a6')
            action['builder'] = {'b': wallet, 'f': self.safe_integer(self.options, 'feeInt', 10)}
