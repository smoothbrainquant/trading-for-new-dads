...
if __name__ == "__main__":
    ...
    if not selected['longs'].empty:
        print(selected['longs'][['symbol', 'close', 'rolling_50d_high', 'rolling_70d_low', 'position']].to_string(index=False))
    else:
    ...
    if not selected['shorts'].empty:
        print(selected['shorts'][['symbol', 'close', 'rolling_50d_low', 'rolling_70d_high', 'position']].to_string(index=False))
    else:
