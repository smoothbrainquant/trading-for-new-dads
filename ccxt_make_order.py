...
def ccxt_make_order(symbol, notional_amount, side, order_type, price=None):
    ...
    try:
        ...
        print(f"  Price: ${effective_price:,.2f}" + (" (specified)" if order_type == 'limit' else " (market)"))
...
Environment Variables Required:
  ...
  HL_SECRET: Your Hyperliquid secret key
    ...
    parser.add_argument('--notional', '-n', type=float, required=True,
                        help='Notional amount in USD/USDC to trade')
    parser.add_argument('--side', '-d', required=True, choices=['buy', 'sell'],
                        help='Order side: buy or sell')
    parser.add_argument('--type', '-t', required=True, choices=['market', 'limit'],
