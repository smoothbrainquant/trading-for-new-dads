...
def main():
    ...
    parser.add_argument(
        "--lookback", type=int, default=30, help="Lookback window for z-score calculation"
    )
    parser.add_argument("--top-n", type=int, default=10, help="Number of LONG positions")
    parser.add_argument("--bottom-n", type=int, default=10, help="Number of SHORT positions")
    parser.add_argument(
        "--output",
        type=str,
        default="signals/oi_divergence_signals.csv",
        help="Output file path for signals",
    )
    parser.add_argument(
        "--current-only", action="store_true", help="Only output current signals (latest date)"
    )
    ...
    if args.current_only:
        ...
        for _, row in long_signals.iterrows():
            print(
                f"  {row['rank']:2d}. {row['symbol']:10s}  Score: {row['score_value']:7.3f}  "
        ...
        print("=" * 80)
        short_signals = signals[signals["position_side"] == "SHORT"].sort_values(
