...
for signal_type in fwd_1d['signal_type'].unique():
    ...
    print("\nBy Period:")
    print(signal_data[['period', 'mean_return', 'sharpe_ratio', 'win_rate_pct', 'count']].to_string(index=False))
...
for period in sorted(fwd_1d['period'].unique()):
    ...
    print(period_data[['signal_type', 'mean_return', 'sharpe_ratio', 'win_rate_pct', 'count']].to_string(index=False))
...
print(f"   Best Period: {int(best_high_vol['period'])}d - Return: {best_high_vol['mean_return']:.4%}, Sharpe: {best_high_vol['sharpe_ratio']:.2f}")

print("\n   Low Volume Signals:")
best_low_vol = low_vol.loc[low_vol['mean_return'].idxmax()]
print(f"   Best Period: {int(best_low_vol['period'])}d - Return: {best_low_vol['mean_return']:.4%}, Sharpe: {best_low_vol['sharpe_ratio']:.2f}")
...
period_perf = fwd_1d.groupby('period').agg({
    'mean_return': 'mean',
    'sharpe_ratio': 'mean',
    'win_rate_pct': 'mean'
...
print(f"   Mid-term (5-10d): Avg Return: {mid_term['mean_return'].mean():.4%}, Avg Sharpe: {mid_term['sharpe_ratio'].mean():.2f}")
print(f"   Long-term (20-30d): Avg Return: {long_term['mean_return'].mean():.4%}, Avg Sharpe: {long_term['sharpe_ratio'].mean():.2f}")
