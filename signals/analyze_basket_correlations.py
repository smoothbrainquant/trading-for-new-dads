...
def load_data():
    ...
    price_df = pd.read_csv(
        DATA_PATH / "combined_coinbase_coinmarketcap_daily.csv", parse_dates=["date"]
    )
    ...
    print(
        f"Loaded {len(category_df):,} category mappings for {category_df['symbol'].nunique()} symbols"
    )
...
def calculate_returns(price_df):
    ...
    returns_pivot = price_df.pivot_table(index="date", columns=symbol_col, values="return")
...
def calculate_basket_returns(price_df, symbols, method="equal_weight"):
    ...
    if method == "equal_weight":
        ...
        basket_returns = basket_data.groupby("date").apply(
            lambda x: (
                x["weighted_return"].sum() / x["market_cap"].sum()
...
def analyze_category(category_name, category_df, price_df, returns_pivot, valid_symbols):
    ...
    summary = {
        ...
        "std_correlation": corr_matrix.where(np.triu(np.ones_like(corr_matrix, dtype=bool), k=1))
        .stack()
        .std(),
    }
...
def main():
    ...
    if len(suitable) > 0:
        ...
        relaxed = summary_df[
            (summary_df["avg_correlation"] > 0.4) & (summary_df["n_symbols"] >= 4)
        ].head(10)
