...
print("-" * 160)
print(
    f"{'Period':<6} {'Any Vol':<14} {'High Vol':<14} {'Low Vol':<14} {'Best':<18} │ "
...
for _, row in df.iterrows():
    ...
    print(
        f"{period:<6} {any_ret:>6} {any_sh:>6}  {high_ret:>6} {high_sh:>6}  {low_ret:>6} {low_sh:>6}  {best_str:<16} │ "
...
print(
    f"  Any Volume:  {df['long_any_vol_return_pct'].mean():.2f}% return, {df['long_any_vol_sharpe'].mean():.2f} Sharpe, {df['long_any_vol_winrate_pct'].mean():.1f}% win rate"
)
print(
    f"  High Volume: {df['long_high_vol_return_pct'].mean():.2f}% return, {df['long_high_vol_sharpe'].mean():.2f} Sharpe, {df['long_high_vol_winrate_pct'].mean():.1f}% win rate"
)
print(
    f"  Low Volume:  {df['long_low_vol_return_pct'].mean():.2f}% return, {df['long_low_vol_sharpe'].mean():.2f} Sharpe, {df['long_low_vol_winrate_pct'].mean():.1f}% win rate"
)
...
for idx, (_, row) in enumerate(top5_df.iterrows(), 1):
    print(
        f"{idx:<6} {row['Config']:<16} {row['Return%']:>11.2f} {row['Sharpe']:>9.2f} {row['WinRate%']:>11.1f} {int(row['Signals']):>9}"
    )
