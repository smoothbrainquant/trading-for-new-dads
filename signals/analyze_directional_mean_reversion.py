...
for _, row in neg_df.iterrows():
    print(f"{row['Period']:<8} {row['Any_Vol_Return']*100:>9.3f} {row['Any_Vol_Sharpe']:>7.2f} {row['Any_Vol_WinRate']:>11.1f} {row['Any_Vol_Count']:>9}")

print("\nHIGH VOLUME (Extreme negative moves with high volume):")
print(f"{'Period':<8} {'Return %':<10} {'Sharpe':<8} {'Win Rate %':<12} {'Signals':<10}")
print("-" * 140)
for _, row in neg_df.iterrows():
    print(f"{row['Period']:<8} {row['HighVol_Return']*100:>9.3f} {row['HighVol_Sharpe']:>7.2f} {row['HighVol_WinRate']:>11.1f} {row['HighVol_Count']:>9}")

print("\nLOW VOLUME (Extreme negative moves with low volume):")
print(f"{'Period':<8} {'Return %':<10} {'Sharpe':<8} {'Win Rate %':<12} {'Signals':<10}")
print("-" * 140)
for _, row in neg_df.iterrows():
    print(f"{row['Period']:<8} {row['LowVol_Return']*100:>9.3f} {row['LowVol_Sharpe']:>7.2f} {row['LowVol_WinRate']:>11.1f} {row['LowVol_Count']:>9}")
...
for _, row in pos_df.iterrows():
    ...
    print(f"{row['Period']:<8} {fwd_return:>13.3f} {short_pnl:>13.3f} {row['Sharpe']:>7.2f} {row['WinRate']:>11.1f} {row['Count']:>9}")
...
for _, row in comp_df.iterrows():
    print(f"{row['Period']:<8} {row['Long_Dips_Return']:>12.3f} {row['Long_Dips_Sharpe']:>9.2f} "
...
print(f"   ✓ Best: {best_long['Period']} high volume → {best_long['HighVol_Return']*100:.2f}% return, {best_long['HighVol_Sharpe']:.2f} Sharpe")
...
print(f"   • Long dips is {comp_df['Long_Dips_Return'].mean() / abs(comp_df['Short_Rallies_PnL'].mean()):.1f}x more profitable")
