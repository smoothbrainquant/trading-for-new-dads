...
def compute_oi_divergence_scores(
    ...
    if not overlap_dates:
        ...
        print(
            f"       Price dates: {min(price_dates)} to {max(price_dates)} ({len(price_dates)} days)"
        )
    ...
    df["d_oi"] = df.groupby("symbol")["oi_close"].transform(
        lambda x: np.log(x) - np.log(x.shift(1))
    )

    df["z_ret"] = df.groupby("symbol")["ret"].transform(lambda s: _zscore(s, window=lookback))
    df["z_doi"] = df.groupby("symbol")["d_oi"].transform(lambda s: _zscore(s, window=lookback))
...
def build_equal_or_risk_parity_weights(
    ...
    if use_risk_parity and (longs or shorts):
        prices = prices.copy()
        prices["ret"] = prices.groupby("symbol")["close"].transform(
            lambda x: np.log(x) - np.log(x.shift(1))
        )
        vol = prices.groupby("symbol")["ret"].transform(
            lambda s: s.rolling(window=volatility_window, min_periods=volatility_window).std()
            * np.sqrt(365)
        )
        prices["vol"] = vol
