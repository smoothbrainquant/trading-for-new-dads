...
pivot_returns = fwd_1d.pivot_table(
    index='period',
    columns='signal_type',
    values='mean_return',
    aggfunc='first'
...
period_summary = fwd_1d.groupby('period').agg({
    'mean_return': 'mean',
    'sharpe_ratio': 'mean',
    'win_rate_pct': 'mean',
    'count': 'sum'
...
top10 = all_signals.nlargest(10, 'sharpe_ratio')[
    ['period', 'signal_type', 'forward_period', 'mean_return', 'sharpe_ratio', 'win_rate_pct', 'count']
]

top10['mean_return_pct'] = top10['mean_return'] * 100
top10 = top10[['period', 'signal_type', 'forward_period', 'mean_return_pct', 'sharpe_ratio', 'win_rate_pct', 'count']]
...
print(f"  Return Difference: {(high_vol['mean_return'].mean() - low_vol['mean_return'].mean()) * 100:.2f}%")
print(f"  Sharpe Difference: {high_vol['sharpe_ratio'].mean() - low_vol['sharpe_ratio'].mean():.2f}")
print(f"  High Vol Outperformance: {(high_vol['mean_return'].mean() / low_vol['mean_return'].mean()):.2f}x")
...
print(f"  Short-term Outperformance: {(short_term['mean_return'].mean() / long_term['mean_return'].mean()):.2f}x")
