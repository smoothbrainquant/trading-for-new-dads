...
def calculate_zscore_divergence(
    coin_returns: pd.Series, basket_returns: pd.Series, window: int = 60
) -> pd.Series:
    """
...
def calculate_percentile_rank(
    coin_returns: pd.Series, all_returns_df: pd.DataFrame, window: int = 20
) -> pd.Series:
    """
    ...
    return (
        percentiles[coin_returns.name]
        if coin_returns.name in percentiles.columns
        else pd.Series(np.nan, index=coin_returns.index)
    )
...
def calculate_correlation_with_basket(
    ...
    # Align series
    aligned = pd.DataFrame({"coin": coin_returns, "basket": basket_returns}).dropna()
...
def apply_liquidity_filter(
    price_df: pd.DataFrame, min_volume: float = MIN_VOLUME_USD, window: int = 30
) -> pd.DataFrame:
...
def apply_market_cap_filter(
    price_df: pd.DataFrame, min_market_cap: float = MIN_MARKET_CAP_USD
) -> pd.DataFrame:
...
def apply_volatility_filter(
    price_df: pd.DataFrame, max_vol: float = MAX_VOLATILITY_ANNUAL, window: int = 30
) -> pd.DataFrame:
...
def apply_data_quality_filter(
    price_df: pd.DataFrame, min_days: int = MIN_DATA_DAYS
) -> pd.DataFrame:
...
def generate_signals_for_category(
    ...
    for symbol in cat_price_df["base"].unique():
        ...
        merged = coin_df[
            ["date", "base", "log_return", "market_cap", "volume_30d", "volatility_30d"]
        ].merge(basket_returns_df, on="date", how="inner")
        ...
        merged["coin_return_20d"] = (
            (1 + merged["log_return"])
            .rolling(20, min_periods=1)
            .apply(lambda x: np.prod(x) - 1, raw=True)
        )
        merged["basket_return_20d"] = (
            (1 + merged["basket_return"])
            .rolling(20, min_periods=1)
            .apply(lambda x: np.prod(x) - 1, raw=True)
        )
        ...
        basket_wide = cat_price_df.pivot_table(index="date", columns="base", values="log_return")
...
def main():
    ...
    parser.add_argument(
        "--all-categories", action="store_true", help="Process all categories (ignore --categories)"
    )
    ...
    # Signals by category
    category_summary = (
        all_signals_df.groupby(["category", "signal"]).size().reset_index(name="count")
    )
    category_summary = category_summary.pivot(
        index="category", columns="signal", values="count"
    ).fillna(0)
