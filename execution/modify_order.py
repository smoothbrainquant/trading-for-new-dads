...
def modify_order(order_id, symbol, new_price=None, new_amount=None):
    ...
    try:
        ...
        print(f"\nModifying order to:")
        print(f"  New Price: ${final_price:,.2f}" + (" (modified)" if new_price is not None else " (unchanged)"))
        print(f"  New Quantity: {final_quantity:.6f}" + (" (modified)" if new_amount is not None else " (unchanged)"))
        if new_amount is not None:
...
Environment Variables Required:
  ...
  HL_SECRET: Your Hyperliquid secret key
    ...
    parser.add_argument('--order-id', '-o', required=True, 
                        help='ID of the order to modify')
    ...
    parser.add_argument('--price', '-p', type=float, default=None,
                        help='New price for the order')
    parser.add_argument('--amount', '-a', type=float, default=None,
                        help='New notional amount in USD/USDC')
    parser.add_argument('--verbose', '-v', action='store_true',
                        help='Show full order details')
