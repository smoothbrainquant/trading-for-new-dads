...
def ccxt_get_hyperliquid_balance():
    ...
    try:
        ...
        return {"perp": perp_balance, "spot": spot_balance}
...
def ccxt_print_balance_summary(balances):
    ...
    if cross_margin_summary and cross_margin_summary != margin_summary:
        ...
        print(
            f"  Total Margin Used:  ${float(cross_margin_summary.get('totalMarginUsed', 0)):,.2f}"
        )
    ...
    if asset_positions:
        ...
        for asset_pos in asset_positions:
            ...
            leverage_val = (
                leverage_info.get("value", 0) if isinstance(leverage_info, dict) else leverage_info
            )
