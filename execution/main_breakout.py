...
from select_insts_breakout import (
    select_instruments_by_breakout_signals,
    get_target_positions_from_signals,
)
...
def calculate_rolling_30d_volatility(data, selected_symbols):
    ...
    for symbol in selected_symbols:
        ...
        if not symbol_data.empty:
            ...
            latest_volatility = (
                symbol_data["volatility_30d"].dropna().iloc[-1]
                if not symbol_data["volatility_30d"].dropna().empty
                else None
            )
...
def calculate_trade_amounts(target_positions, current_positions, notional_value, threshold=0.05):
    ...
    for symbol in all_symbols:
        ...
        if symbol not in target_positions and abs(current) > 0:
            ...
            if side == "short":
                print(
                    f"  ✓ NEUTRALIZE SHORT: Position not in target (BUY ${abs(difference):,.2f} to close)"
                )
            else:
...
def main():
    ...
    target_positions = calculate_target_positions_with_direction(
        weights_long, weights_short, notional_value
    )
    ...
    trades = calculate_trade_amounts(
        target_positions, current_positions, notional_value, threshold=args.rebalance_threshold
    )
