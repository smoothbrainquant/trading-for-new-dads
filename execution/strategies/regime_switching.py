...
Implements direction-aware regime switching based on BTC market conditions:
- Detects market regime using BTC 5-day % change
- Adjusts long/short allocation dynamically based on regime
- Can switch between Trend Following and Mean Reversion strategies
- Uses ADF factor for coin selection

Based on backtest analysis showing:
- Current regime-switching: +42% annualized
- Direction-aware optimization: +80-100% annualized potential
...
def strategy_regime_switching(
    ...
    """
    Regime-switching strategy with direction-aware allocation.
    ...
    """
    print("\n" + "=" * 80)
    print("REGIME-SWITCHING STRATEGY (Direction-Aware)")
    print("=" * 80)
    ...
    if adfuller is None:
        print("  ??  statsmodels not available - cannot calculate ADF")
        return {}
    ...
    try:
        ...
        if btc_symbol is None:
            print("  ??  BTC data not found - cannot detect regime")
            print("  ?? Falling back to balanced allocation (50/50 long/short)")
            regime_name = "Unknown"
            btc_pct_change = 0.0
            long_allocation = 0.5
            ...
            long_allocation, short_allocation, active_strategy = get_optimal_allocation(
        ...
        print(f"     Long Allocation: {long_allocation*100:.0f}%")
        print(f"     Short Allocation: {short_allocation*100:.0f}%")
        ...
        if adf_df.empty:
            print("  ??  No symbols with valid ADF calculations")
            return {}
        ...
        print(f"\n  ? Error in regime-switching strategy: {str(e)}")
        import traceback
        traceback.print_exc()
        return {}
...
if __name__ == "__main__":
    print("Regime-Switching Strategy Module")
    print("=" * 80)
    print("\nThis module implements direction-aware regime switching.")
    print("\nKey Features:")
