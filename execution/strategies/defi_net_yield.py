"""
Net Yield Factor Strategy

Strategy: Long coins with highest net yield, short coins with lowest net yield
Net Yield: Fee Yield - Emission Yield (%)

This is the TRUE value capture metric combining revenue and dilution:
- Positive net yield = value accruing to holders
- Negative net yield = value being extracted from holders
...
Worst performers:
- PENDLE: -14.46% (emitting faster than earning!)

Key Insight: Net yield shows the real economics - fees earned minus dilution cost.
"""
...
def strategy_defi_net_yield(
    ...
    """
    DeFi Net Yield factor strategy - Long high net yield, Short low net yield.
    ...
    """
    print("\n" + "=" * 80)
    print("DEFI NET YIELD FACTOR STRATEGY")
    print("=" * 80)
    print(f"Parameters: top_n={top_n}, bottom_n={bottom_n}, rebalance={rebalance_days}d, weighting={weighting_method}")
    print("Net Yield = Fee Yield - Emission Yield (true value capture)")
    
    # Load latest net yield signals
    signal_file = '/workspace/signals/defi_factor_net_yield_signals.csv'
    if not os.path.exists(signal_file):
        print(f"\n⚠️ Net yield signals not found: {signal_file}")
    ...
    try:
        ...
        print(f"\n⚠️ Error loading net yield signals: {e}")
        return {}
    
    print(f"\nLoaded {len(signals_df)} coins with net yield data")
    print(f"Date: {signals_df['date'].iloc[0] if 'date' in signals_df.columns else 'unknown'}")
    
    # Filter for valid data
    signals_df = signals_df[signals_df['net_yield_pct'].notna()].copy()
    ...
    print(f"Found {len(signals_df)} tradeable coins with net yield data")
    ...
    # Sort by net yield
    signals_df = signals_df.sort_values('net_yield_pct', ascending=False)
    
    # Select top and bottom
    top_net_yield = signals_df.head(top_n).copy()
    bottom_net_yield = signals_df.tail(bottom_n).copy()
    
    print(f"\nTop {top_n} Net Yield (LONG):")
    for idx, row in top_net_yield.iterrows():
        print(f"  {row['symbol']:10s} -> {row['trading_symbol']:12s}: {row['net_yield_pct']:.2f}%")
    
    print(f"\nBottom {bottom_n} Net Yield (SHORT):")
    for idx, row in bottom_net_yield.iterrows():
        print(f"  {row['symbol']:10s} -> {row['trading_symbol']:12s}: {row['net_yield_pct']:.2f}%")
    
    # Calculate volatilities for risk parity weighting
    volatilities = {}
    for symbol in list(top_net_yield['trading_symbol']) + list(bottom_net_yield['trading_symbol']):
    ...
    # Short positions (low net yield)
    short_notional = notional * short_allocation
    if weighting_method == "risk_parity" and volatilities:
        # Risk parity: inverse volatility weighting
        short_symbols = list(bottom_net_yield['trading_symbol'])
