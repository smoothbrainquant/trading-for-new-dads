...
def _compute_latest_return_and_volume_zscores(
    ...
    df_all["ret_mean"] = df_all.groupby("symbol")["return_nd"].transform(
        lambda x: x.rolling(lookback_window, min_periods=lookback_window).mean().shift(1)
    )
    df_all["ret_std"] = df_all.groupby("symbol")["return_nd"].transform(
        lambda x: x.rolling(lookback_window, min_periods=lookback_window).std().shift(1)
    )

    # Rolling mean/std for volume (shifted to avoid look-ahead bias)
    df_all["vol_mean"] = df_all.groupby("symbol")["volume_change_nd"].transform(
        lambda x: x.rolling(lookback_window, min_periods=lookback_window).mean().shift(1)
    )
    df_all["vol_std"] = df_all.groupby("symbol")["volume_change_nd"].transform(
        lambda x: x.rolling(lookback_window, min_periods=lookback_window).std().shift(1)
    )
...
def strategy_mean_reversion(
    ...
    long_candidates = [
        ...
        if (
            return_z_map.get(s, float("nan")) <= -abs(zscore_threshold)
            and abs(volume_z_map.get(s, float("nan"))) >= volume_threshold
        )
    ]

    if not long_candidates:
        print(
            f"  No mean reversion LONG candidates (return z < -{zscore_threshold:.1f}, |volume z| >= {volume_threshold:.1f})"
        )
        return {}
    ...
    print(
        f"  MeanRev LONGS: {len(long_candidates)} selected ({period_days}d return z <= -{zscore_threshold:.1f}, high volume)"
    )
