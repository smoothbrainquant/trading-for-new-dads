...
def move_order_to_best_price(exchange, order: dict, bid_ask_dict: Dict[str, Dict[str, float]], 
    ...
    if current_price and target_price:
        ...
        if price_diff_pct < 0.01:
            return {
                'status': 'skip',
                'message': f'Already at {price_type} (within 0.01%)'
            }
    
    # Calculate price change
    price_change = target_price - current_price if current_price else 0
    price_change_pct = (price_change / current_price * 100) if current_price and current_price > 0 else 0
...
def tick_orders(dry_run: bool = True, verbose: bool = False):
    ...
    try:
        ...
        if not bid_ask_dict:
            ...
            return {
                'success': False,
                'error': 'No bid/ask data available'
            }
        ...
        for i, order in enumerate(orders, 1):
            ...
            print(f"  Current Price: ${current_price:,.8f}" if current_price else "  Current Price: N/A")
...
Safety:
  ...
  - Orders within 0.01% of target price are skipped
    ...
    parser.add_argument(
        '--live',
        action='store_true',
        help='Execute live order modifications (default is dry-run)'
    )
    parser.add_argument(
        '--verbose',
        '-v',
