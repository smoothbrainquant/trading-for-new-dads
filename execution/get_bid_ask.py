    ...
    for symbol in symbols:
        try:
            ...
            if bid is not None and ask is not None:
                ...
                print(
                    f"{symbol:20s} | Bid: ${bid:>12,.8f} | Ask: ${ask:>12,.8f} | Spread: ${spread:>8.8f} ({spread_pct:.4f}%)"
                )
            else:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--symbols", nargs="+", help="List of symbols to fetch (e.g., BTC/USDC:USDC ETH/USDC:USDC)"
    )
    parser.add_argument("--csv", type=str, help="Save results to CSV file")
    parser.add_argument("--all", action="store_true", help="Fetch bid/ask for all active markets")
