...
def analyze_funding_rates(df):
    ...
    top_10 = df.nlargest(10, 'funding_rate_pct')[['date', 'coin_name', 'coin_symbol', 'funding_rate_pct']]
    print(top_10.to_string(index=False))
    
    print("\nTop 10 Lowest (Most Negative) Funding Rates:")
    bottom_10 = df.nsmallest(10, 'funding_rate_pct')[['date', 'coin_name', 'coin_symbol', 'funding_rate_pct']]
    ...
    top_10_coins = df[df['rank'] <= 10].groupby(['rank', 'coin_name', 'coin_symbol']).agg({
    ...
    positive_pct = df[df['funding_rate_pct'] > 0].groupby('coin_symbol').size() / df.groupby('coin_symbol').size() * 100
    ...
    volatility = df.groupby(['coin_name', 'coin_symbol'])['funding_rate_pct'].std().sort_values(ascending=False)
    ...
    high_funding = last_7_days[last_7_days['funding_rate_pct'] > 1.0].groupby('coin_symbol').agg({
    ...
    low_funding = last_7_days[last_7_days['funding_rate_pct'] < -0.5].groupby('coin_symbol').agg({
        'funding_rate_pct': 'mean',
        'coin_name': 'first'
