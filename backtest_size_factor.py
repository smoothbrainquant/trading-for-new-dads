...
def backtest(
    ...
    for bucket in range(1, num_buckets + 1):
        ...
        if len(bucket_df) > 0:
            print(f"  Bucket {bucket} ({'Large' if bucket == 1 else 'Small' if bucket == num_buckets else 'Mid'}): "
    ...
    for date_idx, current_date in enumerate(daily_tracking_dates):
        ...
        if is_rebalance_day:
            ...
            long_symbols_trading = historical_data[
                historical_data['base_symbol'].isin(long_symbols_base)
            ]['symbol'].unique().tolist()
            
            short_symbols_trading = historical_data[
                historical_data['base_symbol'].isin(short_symbols_base)
            ]['symbol'].unique().tolist()
...
def print_results(results):
    ...
    print(f"  Long symbols ({len(strategy_info['long_symbols'])}): {', '.join(strategy_info['long_symbols'][:10])}" + 
          (f" ... (+{len(strategy_info['long_symbols'])-10} more)" if len(strategy_info['long_symbols']) > 10 else ""))
    print(f"  Short symbols ({len(strategy_info['short_symbols'])}): {', '.join(strategy_info['short_symbols'][:10])}" + 
          (f" ... (+{len(strategy_info['short_symbols'])-10} more)" if len(strategy_info['short_symbols']) > 10 else ""))
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=5,
        help='Number of size buckets for ranking (quintiles)'
    )
    parser.add_argument(
        '--volatility-window',
        type=int,
