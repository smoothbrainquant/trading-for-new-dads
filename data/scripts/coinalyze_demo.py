...
def main():
    ...
    if futures:
        ...
        for perp in btc_perps[:10]:
            exch = next((e['name'] for e in client.get_exchanges() if e['code'] == perp['exchange']), 'Unknown')
            print(f"{perp['symbol']:<25} {exch:<15} {str(perp.get('has_ohlcv_data', False)):<15} {perp.get('has_long_short_ratio_data', False)}")
        ...
        if binance_symbols:
            ...
            if fr_data:
                ...
                for item in fr_data:
                    ...
                    update_time = datetime.fromtimestamp(item['update'] / 1000).strftime('%Y-%m-%d %H:%M:%S')
    ...
    if binance_symbols:
        ...
        if oi_hist and len(oi_hist) > 0:
            ...
            for point in history:
                ...
                print(f"{date:<15} {point['o']:>12,.0f}   {point['h']:>12,.0f}   {point['l']:>12,.0f}   {point['c']:>12,.0f}")
    ...
    if binance_symbols:
        ...
        if ohlcv and len(ohlcv) > 0:
            ...
            print(f"{'Date':<15} {'Open':<12} {'High':<12} {'Low':<12} {'Close':<12} {'Volume':<15}")
            print("-" * 80)
            
            for candle in history[-5:]:  # Last 5 days
                date = datetime.fromtimestamp(candle['t']).strftime('%Y-%m-%d')
                print(f"{date:<15} {candle['o']:>10.2f}   {candle['h']:>10.2f}   {candle['l']:>10.2f}   {candle['c']:>10.2f}   {candle['v']:>12,.2f}")
