...
def main():
    ...
    export_df['symbols'] = export_df['symbols'].apply(lambda x: ', '.join([str(s) for s in x]) if isinstance(x, list) else str(x))
    export_df['columns'] = export_df['columns'].apply(lambda x: ', '.join(x) if isinstance(x, list) else str(x))
    ...
    print("\n📊 By API Source:")
    api_summary = df_summary.groupby('api').agg({
        'filename': 'count',
        'file_size_mb': 'sum',
        'n_rows': 'sum',
        'n_symbols': 'sum'
    ...
    print("\n📊 By Data Type:")
    type_summary = df_summary.groupby('type').agg({
        'filename': 'count',
        'file_size_mb': 'sum',
        'n_rows': 'sum'
    }).round(2)
