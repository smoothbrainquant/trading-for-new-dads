#!/usr/bin/env python3
"""
Fetch Historical Open Interest (OI) for Top 50 Coins
Uses Coinalyze API + CoinMarketCap snapshots
- Pulls top 50 coins by latest CMC snapshot
- Maps to Coinalyze perpetual symbols (prioritizes Binance/Bybit/OKX)
- Fetches last 90 days of DAILY OI (USD) as OHLC (o,h,l,c)
- Saves detailed CSV and summary CSV
"""
import os
import logging
import time
...
def find_perpetual_symbols(client: CoinalyzeClient, coin_symbols: List[str]) -> Dict[str, str]:
    ...
    Only USDT/USD/USDC quote assets considered.
    """
...
def fetch_oi_history(
    ...
    """
    Fetch historical OI (USD) for given symbols.
    ...
    logger.info(f"Time range: {days} days, interval: {interval}")
...
def main():
    print("=" * 80)
    print("FETCH HISTORICAL OPEN INTEREST (OI) - TOP 50 COINS")
    print("=" * 80)
    ...
    # Step 3: Fetch OI history (daily, USD)
    coinalyze_symbols = list(symbol_map.values())
    oi_df = fetch_oi_history(client, coinalyze_symbols, days=90, interval='daily', convert_to_usd=True)
    ...
    detailed_file = out_dir / f'historical_open_interest_top50_{timestamp}.csv'
    ...
    summary_file = out_dir / f'historical_open_interest_top50_summary_{timestamp}.csv'
