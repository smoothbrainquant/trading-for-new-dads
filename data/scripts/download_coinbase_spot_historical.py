...
def get_unique_symbols_from_snapshots(
    filename: str = "coinmarketcap_historical_all_snapshots.csv",
) -> List[str]:
    """
            ...
            if market.get("type") == "spot" and market.get("active", True):
                ...
                if quote in ["USD", "USDT", "USDC"]:
                    ...
                    if base not in spot_markets or (
                        quote == "USD"
                        or (quote == "USDT" and "/USD" not in spot_markets.get(base, ""))
                        or (
                            quote == "USDC"
                            and "/USD" not in spot_markets.get(base, "")
                            and "/USDT" not in spot_markets.get(base, "")
                        )
                    ):
...
def fetch_daily_data_for_symbol(
    exchange: ccxt.Exchange, symbol: str, preferred_start_date: datetime = None
) -> pd.DataFrame:
    """
    ...
    try:
            ...
            while current_date < now:
                ...
                try:
                    ohlcv = exchange.fetch_ohlcv(
                        symbol=symbol, timeframe="1d", since=since, limit=chunk_days
                    )
        ...
        if not all_ohlcv:
            try:
                ohlcv = exchange.fetch_ohlcv(
                    symbol=symbol, timeframe="1d", limit=1000  # Get last 1000 days
                )
                if ohlcv:
                    all_ohlcv = ohlcv
            except:
...
def download_coinbase_historical_data(
    start_date: str = "2020-01-01", output_filename: str = None
) -> pd.DataFrame:
    """
    ...
    if all_data:
        ...
        if output_filename is None:
            ...
            output_filename = (
                f'coinbase_spot_daily_data_{start_date.replace("-", "")}_{timestamp}.csv'
            )
...
if __name__ == "__main__":
    ...
    df = download_coinbase_historical_data(start_date="2020-01-01")
    ...
    if not df.empty:
        ...
        print("=" * 80)
        summary = df.groupby("base").agg({"date": ["min", "max", "count"]}).reset_index()
