"""
Fetch Monthly CoinMarketCap Historical Snapshots for 2020-2022

This script downloads monthly historical snapshot data from CoinMarketCap
for 2020 (Jan-Dec), 2021 (Jan-Dec), and 2022 (Jan-Dec).
"""
...
def fetch_historical_snapshot(date_str):
    ...
    Args:
        date_str (str): Date string in format YYYYMMDD (e.g., '20200101')
...
def save_snapshot_to_csv(df, date_str, output_dir="../raw"):
    """
...
def main():
    ...
    print("=" * 80)
    print("COINMARKETCAP MONTHLY HISTORICAL SNAPSHOTS DOWNLOAD (2020-2022)")
    print("=" * 80)

    # Generate monthly dates for 2020, 2021, and 2022
    print("\nGenerating monthly snapshots for:")
    print("  - 2020: January - December (12 months)")
    print("  - 2021: January - December (12 months)")
    print("  - 2022: January - December (12 months)")

    snapshot_dates_2020 = generate_monthly_dates_for_period(2020, 1, 2020, 12)
    snapshot_dates_2021 = generate_monthly_dates_for_period(2021, 1, 2021, 12)
    snapshot_dates_2022 = generate_monthly_dates_for_period(2022, 1, 2022, 12)

    snapshot_dates = snapshot_dates_2020 + snapshot_dates_2021 + snapshot_dates_2022

    print(f"\nTotal snapshot dates to fetch: {len(snapshot_dates)}")
    print("\n2020 Dates:")
    for date in snapshot_dates_2020:
        year = date[:4]
        month = date[4:6]
    ...
    print("\n2021 Dates:")
    for date in snapshot_dates_2021:
        year = date[:4]
        month = date[4:6]
        dt = datetime.strptime(f"{year}{month}01", "%Y%m%d")
        print(f"  - {dt.strftime('%B %Y')} ({date})")
    ...
    print("\n2022 Dates:")
    for date in snapshot_dates_2022:
        year = date[:4]
        month = date[4:6]
        dt = datetime.strptime(f"{year}{month}01", "%Y%m%d")
        print(f"  - {dt.strftime('%B %Y')} ({date})")
    ...
    if all_snapshots:
        print("\n" + "=" * 80)
        print("COMBINING ALL MONTHLY SNAPSHOTS (2020-2022)")
        print("=" * 80)

        combined_df = pd.concat(all_snapshots, ignore_index=True)
        combined_filepath = "../raw/coinmarketcap_monthly_2020_2022_snapshots.csv"
    ...
    print(f"\nIndividual monthly files are saved as: ../raw/coinmarketcap_monthly_YYYYMM01.csv")
    print(f"Combined file saved as: ../raw/coinmarketcap_monthly_2020_2022_snapshots.csv")
