...
def download_binance_daily_data(
    ...
    if symbols is None:
        ...
        symbols = get_top_binance_pairs(
            exchange, market_type=market_type, limit=20, exchange_name=exchange_name
        )
            ...
            if exchange_name == "bybit":
                if market_type == "spot":
                    ...
                    if "/USDT" in symbol and ":" not in symbol:
                        volume_data.append(
                            {"symbol": symbol, "volume": ticker.get("quoteVolume", 0) or 0}
                        )
                else:  # futures
...
def download_specific_symbols(
    symbols, days=500, market_type="spot", output_file=None, exchange_name="bybit"
):
    """
...
def download_top_volume_pairs(
    limit=20, days=500, market_type="spot", output_file=None, exchange_name="bybit"
):
    """
    ...
    symbols = get_top_binance_pairs(
        exchange, market_type=market_type, limit=limit, exchange_name=exchange_name
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--days", type=int, default=500, help="Number of days of historical data to download"
    )
    parser.add_argument(
        "--market-type",
    ...
    parser.add_argument(
        "--output", type=str, help="Output CSV filename (auto-generated if not provided)"
    )
    parser.add_argument(
        "--exchange",
        type=str,
