#!/usr/bin/env python3
"""
Fetch ALL Historical Funding Rates - Top 100 Coins - Maximum Available Data
Fetches daily funding rates from 2019 to present for top 100 coins
"""
import pandas as pd
import os
...
def get_top_100_coins():
    """Load top 100 coins from latest CoinMarketCap snapshot"""
    logger.info("Loading CoinMarketCap data...")

    df = pd.read_csv("../raw/coinmarketcap_historical_all_snapshots.csv")
    latest_date = df["snapshot_date"].max()
    logger.info(f"Latest snapshot: {latest_date}")

    latest = df[df["snapshot_date"] == latest_date].copy()
    latest = latest.sort_values("Rank").head(100)

    logger.info(f"Found {len(latest)} coins in top 100")
...
def fetch_max_history_for_symbol(client, symbol, start_year=2019):
    ...
    logger.info(
        f"  Fetching {symbol} from {datetime.fromtimestamp(start_ts).date()} to {datetime.fromtimestamp(end_ts).date()}"
    )
...
def main():
    print("=" * 80)
    print("FETCH ALL HISTORICAL FUNDING RATES - TOP 100 COINS - MAXIMUM AVAILABLE DATA")
    print("=" * 80)
    ...
    print("\nTop 100 Coins:")
    print("-" * 80)
    for coin in top_100[:10]:
        print(f"  {coin['Rank']:>2}. {coin['Name']:<30} ({coin['Symbol']})")
    print(f"  ... and {len(top_100) - 10} more")

    # Step 2: Map to Coinalyze symbols
    coin_symbols = [coin["Symbol"] for coin in top_100]
    ...
    funding_df = fetch_all_funding_rates_max_history(
        client, coinalyze_symbols, start_year=2019  # Most perpetuals launched 2019-2020
    )
    ...
    coin_info_map = {coin["Symbol"]: coin for coin in top_100}
    ...
    output_file = f"historical_funding_rates_top100_ALL_HISTORY_{timestamp}.csv"
    ...
    print(
        f"Total days of data: {(pd.to_datetime(funding_df['date'].max()) - pd.to_datetime(funding_df['date'].min())).days}"
    )
    ...
    availability_file = f"funding_rates_data_availability_top100_{timestamp}.csv"
    ...
    # Save summary
    summary_file = f"historical_funding_rates_top100_ALL_HISTORY_summary_{timestamp}.csv"
    ...
    print(
        f"{(pd.to_datetime(funding_df['date'].max()) - pd.to_datetime(funding_df['date'].min())).days} days"
    )
