...
def get_cmc_symbols() -> Set[str]:
    ...
    if os.path.exists(historical_file):
        ...
        print(
            f"  Date range: {df_historical['snapshot_date'].min()} to {df_historical['snapshot_date'].max()}"
        )
    else:
    ...
    if os.path.exists(monthly_file):
        ...
        print(
            f"  Date range: {df_monthly['snapshot_date'].min()} to {df_monthly['snapshot_date'].max()}"
        )
    else:
            ...
            if is_spot:
                ...
                if quote in ["USD", "USDT", "USDC"]:
                    ...
                    if base not in spot_markets:
                        ...
                        quote == "USDT"
                        and "/USD" not in current_symbol
                        and "/USDT" not in current_symbol
                    ):
                        spot_markets[base] = symbol
...
def fetch_daily_data_for_symbol(
    exchange: ccxt.Exchange, symbol: str, start_date: datetime
) -> pd.DataFrame:
    """
    ...
    try:
        ...
        if not all_ohlcv:
            try:
                ohlcv = exchange.fetch_ohlcv(symbol=symbol, timeframe="1d", limit=1000)
                if ohlcv:
                    all_ohlcv = ohlcv
            except:
                pass
...
def download_missing_symbols(
    missing_symbols: Set[str], start_date: str = "2020-01-01"
) -> pd.DataFrame:
    """
    ...
    if all_data:
        ...
        summary = combined_df.groupby("base").agg({"date": ["min", "max", "count"]}).reset_index()
