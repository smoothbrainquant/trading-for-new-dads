...
def get_cmc_coinbase_symbols() -> Set[str]:
    ...
    cmc_historical = os.path.join(workspace_root, 'data/raw/coinmarketcap_historical_all_snapshots.csv')
    ...
    coinbase_combined = os.path.join(workspace_root, 'data/raw/combined_coinbase_coinmarketcap_daily.csv')
    ...
    coinbase_spot = os.path.join(workspace_root, 'data/raw/coinbase_spot_daily_data_20200101_20251024_110130.csv')
...
def get_funding_rate_symbols() -> Set[str]:
    ...
    funding_files = [
        os.path.join(workspace_root, 'data/raw/historical_funding_rates_top100_20251025_124832.csv'),
        os.path.join(workspace_root, 'data/raw/historical_funding_rates_top50_ALL_HISTORY_20251025_123832.csv'),
    ]
            ...
            try:
                ...
                if result:
                    ...
                    if retry_count < max_retries:
                        ...
                        logger.warning(f"  ⚠ Failed to fetch batch, retrying in {wait_time}s (attempt {retry_count+1}/{max_retries})...")
                ...
                if retry_count < max_retries:
                    wait_time = 10 * retry_count
                    logger.warning(f"  ⚠ Error: {e}, retrying in {wait_time}s (attempt {retry_count+1}/{max_retries})...")
                    time.sleep(wait_time)
                else:
...
def download_missing_funding_rates(
    ...
    funding_df = funding_df[[
        'coin_symbol', 'symbol', 'date', 'timestamp', 
    ...
    workspace_root = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..'))
    output_file = os.path.join(workspace_root, f'data/raw/funding_rates_missing_symbols_{timestamp}.csv')
    ...
    summary = funding_df.groupby(['coin_symbol', 'symbol']).agg({
    ...
    summary.columns = [
        'Coin Symbol', 'Coinalyze Symbol', 
        'Data Points', 'Avg FR %', 'Std FR %', 'Min FR %', 'Max FR %',
    ...
    # Save summary
    summary_file = os.path.join(workspace_root, f'data/raw/funding_rates_missing_symbols_summary_{timestamp}.csv')
...
def main():
    ...
    if missing_symbols:
        ...
        print(f"{'='*80}")
        print(f"Will attempt to download funding rate history for {len(missing_symbols)} missing symbols")
