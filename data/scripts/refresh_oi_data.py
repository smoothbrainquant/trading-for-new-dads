    ...
    try:
        ...
        for i, (base, c_sym) in enumerate(sorted(base_to_symbol.items()), 1):
            ...
            if i % 10 == 0 or i == 1:
                ...
                print(f"  [{i:4d}/{total_symbols}] {base:8s}: {c_sym:25s} | Elapsed: {elapsed:.1f}m | Remaining: {remaining:.1f}m")
...
def main():
    ...
    parser.add_argument(
        '--force',
        action='store_true',
        help='Force download even if data is fresh'
    )
    parser.add_argument(
        '--check-only',
        action='store_true',
