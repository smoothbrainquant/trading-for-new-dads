...
def analyze_categories(categories_df):
    ...
    category_counts = categories_df.groupby('category').agg({
        'symbol': 'count',
    ...
    print(f"Avg assignments per symbol: {len(categories_df) / categories_df['symbol'].nunique():.2f}")
...
def analyze_category_overlap(categories_df):
        ...
        for cat2 in categories[i+1:]:
            ...
            if len(overlap) > 0:
                overlaps.append({
                    ...
                    'overlap_pct': 100 * len(overlap) / min(len(symbols_by_category[cat1]), len(symbols_by_category[cat2]))
                })
    ...
    print(overlap_df.head(20)[['category_1', 'category_2', 'overlap_count', 'overlap_pct']].to_string(index=False))
...
def generate_category_file_for_backtest(categories_df, available_symbols, output_file='data/raw/category_mappings_validated.csv'):
...
def main():
    ...
    print(f"✓ Symbols with valid price data: {len([s for s in categories_df['symbol'].unique() if s in available_symbols])}")
