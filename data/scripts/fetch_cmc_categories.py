...
def analyze_coverage(memberships_df, available_symbols):
    ...
    print(
        f"Overlap: {len(overlap)} symbols ({100*len(overlap)/len(available_set):.1f}% of our data)"
    )
...
def main():
    ...
    parser.add_argument(
        "--api-key", type=str, default=None, help="CoinMarketCap API key (or set CMC_API env var)"
    )
    parser.add_argument("--limit", type=int, default=100, help="Number of categories to fetch")
    parser.add_argument(
        "--fetch-members",
        action="store_true",
    ...
    parser.add_argument(
        "--output-dir", type=str, default="data/raw", help="Output directory for CSV files"
    )
    ...
    if len(categories_df) > 0:
        print("\nCategory Summary:")
        print(
            categories_df[["category_name", "num_tokens", "market_cap", "volume"]]
            .head(20)
